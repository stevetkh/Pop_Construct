...
#compile("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_spline_RW_aggr_gumbel_common_AR2_all.cpp")
...
full.penal.gx.AR2.tensor <- as(AR2.PREC(no.basis.time, c(2*0.9, -0.9)) %x% AR2.PREC(no.basis.age, c(2*0.9, -0.9)), "sparseMatrix")
...
gumbel.theta.phi <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(1.1)
gumbel.theta.psi <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(1.1)
gumbel.theta.A <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(1.1)
gumbel.theta.B <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(1.1)

gumbel.theta.lambda <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(3)
gumbel.theta.delta <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(3)
gumbel.theta.epsilon <- -log(0.01) * sqrt(mean(diag(A.year %*% solve(full.penal.time) %*% t(A.year)))) * 1.96 / log(3)
...
# get.tau <- function(tau, max.df, tau.e, v.values) {
#   (max.df - sum(1/(1 + (tau/tau.e) * v.values)))^2
# }
# 
# gumbel.theta.fx <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 2*2, tau.e = 1.96^2/log(1.5)^2, v.values = eigen(solve(crossprod(te.spline.fert)))$values)$par)
# gumbel.theta.gx <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 7, tau.e = 1.96^2/0.08^2, v.values = eigen(full.penal.gx %*% solve(crossprod(te.spline)))$values)$par)
# 
# gumbel.theta.phi <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 1, tau.e = 1.96^2/log(1.3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# gumbel.theta.psi <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 1.5, tau.e = 1.96^2/log(1.3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# gumbel.theta.A <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 1, tau.e = 1.96^2/log(1.3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# gumbel.theta.B <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 1.5, tau.e = 1.96^2/log(1.3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# 
# gumbel.theta.lambda <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 5, tau.e = 1.96^2/log(3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# gumbel.theta.delta <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 5, tau.e = 1.96^2/log(2)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
# gumbel.theta.epsilon <- -log(0.01) * sqrt(optim(2, get.tau, method="Brent", upper=1e10, lower=0, max.df = 5, tau.e = 1.96^2/log(3)^2, v.values = eigen(full.penal.time %*% solve(crossprod(A.year)))$values)$par)
...
par.vec <- list(log_tau2_logpop_f = c(-2*log(log(1.5)/1.96),-2*log(log(1.5)/1.96)), log_tau2_logpop_m = c(-2*log(log(1.5)/1.96),-2*log(log(1.5)/1.96)),
                ...
                # log_lambda_phi = rep(log((gumbel.theta.phi/-log(0.01))^2) + 0.1 , 2),
                # log_lambda_psi = rep(log((gumbel.theta.psi/-log(0.01))^2) + 0.1 , 2),
                # log_lambda_A = rep(log((gumbel.theta.A/-log(0.01))^2) + 0.1 , 2),
                # log_lambda_B = rep(log((gumbel.theta.B/-log(0.01))^2) + 0.1 , 2),
                # 
                # log_lambda_lambda = rep(log((gumbel.theta.lambda/-log(0.01))^2) + 0.1 , 2),
...
# obj <- make_tmb_obj(data = input.thiele.loghump.oag.vec.RW$data,
#                     par = input.thiele.loghump.oag.vec.RW$par_init,
#                     model = input.thiele.loghump.oag.vec.RW$model,
#                     inner_verbose = TRUE,
#                     calc_outputs = 0L,
...
#                     DLL="ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_spline_RW_aggr_gumbel",
#                     map = list(log_tau2_logpop = factor(c(1, 2, 3, 4)),
#                                log_lambda_gx = factor(c(1,1)),
#                                log_lambda_phi = factor(c(1,1)),
#                                log_lambda_psi = factor(c(1,1)),
#                                log_lambda_A = factor(c(1,1)),
#                                log_lambda_B = factor(c(1,1)),
#                                log_lambda_lambda = factor(c(1,1)),
#                                log_lambda_delta = factor(c(1,1)),
...
system.time(thiele.f.loghump.oag.RW.ori <- fit_tmb(input.thiele.loghump.oag.vec.RW, inner_verbose=TRUE,
                                                   random = c("log_basepop_f","log_basepop_m",
                                                              ...
                                                              "log_A_f_spline_params", "log_A_m_spline_params",
                                                              "log_B_f_spline_params", "log_B_m_spline_params"),
                                                   DLL="ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_spline_RW_aggr_gumbel_common_AR2_phi_all",
