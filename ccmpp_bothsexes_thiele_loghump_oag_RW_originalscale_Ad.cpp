...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(log_marginal_prec_A_d);
 ...
 //mode = exp(log_hypervar_prec)
  ...
  nll -= dlgamma(log_marginal_prec_A_d, Type(1.0), Type(exp(log_A_hypervar_prec) / 2.0), true);
  Type sigma_A_d(exp(-0.5 * log_marginal_prec_A_d));
  ...
  nll -= dnorm(logit_rho_A_d, Type(1.1), Type(3.0), 1);
  ...
  Type rho_A_d = 2.0 * invlogit(logit_rho_A_d) - 1.0;
  ...
  nll += SCALE(AR1(rho_A_d), sigma_A_d)(log_A_d - log_A_mean_d);
  ...
  vector<Type> A_m = exp(log_A_f) + exp(log_A_d);
