...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(hiv_child_mort);
  ...
  nll -= dnorm(hiv_child_mort, Type(0.0), Type(2.0), 1).sum();
  nll -= dnorm(k_constant_f, Type(0.0), Type(5.0), 1);
  ...
  nll += SCALE(AR1(rho_h), sigma_h)(h_params_f - h_constant_f + hiv_child_mort);
  nll += SCALE(AR1(rho_k), sigma_k)(k_params_f - k_constant_f);
  nll += SCALE(AR1(rho_D), sigma_D)(log_D - log_D_mean);
  nll += SCALE(AR1(rho_E), sigma_E)(log_E - log_E_mean);
  nll += SCALE(AR1(rho_F), sigma_F)(log_F - log_F_mean);
