...
Type objective_function<Type>::operator() ()
{
  ...
  Type d_gamma_lambda = d_gamma(gamma_lambda, rho_lambda, no_basis_time - Type(hiv_var_cut));
  Type d_gamma_delta= d_gamma(gamma_delta, rho_delta, no_basis_time - Type(hiv_var_cut));
  Type d_gamma_epsilon = d_gamma(gamma_epsilon, rho_epsilon, no_basis_time - Type(hiv_var_cut));

  Type theta_gamma_lambda = -log(0.01) / d_gamma(upper_gamma, rho_lambda, no_basis_time - Type(hiv_var_cut));
  Type theta_gamma_delta = -log(0.01) / d_gamma(upper_gamma, rho_delta, no_basis_time - Type(hiv_var_cut));
  Type theta_gamma_epsilon = -log(0.01) / d_gamma(upper_gamma, rho_epsilon, no_basis_time - Type(hiv_var_cut));

  nll -= dexp(d_gamma_lambda, theta_gamma_lambda, 1);
  nll -= dexp(d_gamma_delta, theta_gamma_delta, 1);
  nll -= dexp(d_gamma_epsilon, theta_gamma_epsilon, 1);

  nll -= logdd_gamma(gamma_lambda, rho_lambda, no_basis_time- Type(hiv_var_cut));
  nll -= logdd_gamma(gamma_delta, rho_delta, no_basis_time- Type(hiv_var_cut));
  nll -= logdd_gamma(gamma_epsilon, rho_epsilon, no_basis_time- Type(hiv_var_cut));
  ...
  vector<Type> sigma_lambda(no_basis_time_int);
  vector<Type> sigma_delta(no_basis_time_int);
  vector<Type> sigma_epsilon(no_basis_time_int);

  for(int i = 0; i < hiv_var_cut; i++){
    sigma_lambda(i) = exp(-0.5 * log_marginal_lambda_lambda);
    sigma_delta(i) = exp(-0.5 * log_marginal_lambda_delta);
    sigma_epsilon(i) = exp(-0.5 * log_marginal_lambda_epsilon);
  }

  for(int i = 0; i < no_basis_time_int - hiv_var_cut; i++){
    sigma_lambda(hiv_var_cut + i) = exp(-0.5 * log_marginal_lambda_lambda) * sqrt(gamma_lambda);
    sigma_delta(hiv_var_cut + i) = exp(-0.5 * log_marginal_lambda_delta) * sqrt(gamma_delta);
    sigma_epsilon(hiv_var_cut + i) = exp(-0.5 * log_marginal_lambda_epsilon) * sqrt(gamma_epsilon);
  }
  ...
  nll += VECSCALE(ARk(vector<Type>(rho_lambda * rho_vec)), sigma_lambda)(log_lambda_m_spline_params) + VECSCALE(ARk(vector<Type>(rho_lambda * rho_vec)), sigma_lambda)(log_lambda_f_spline_params);
  nll += VECSCALE(ARk(vector<Type>(rho_delta * rho_vec)), sigma_delta)(log_delta_m_spline_params) + VECSCALE(ARk(vector<Type>(rho_delta * rho_vec)), sigma_delta)(log_delta_f_spline_params);
  nll += VECSCALE(ARk(vector<Type>(rho_epsilon * rho_vec)), sigma_epsilon)(log_epsilon_m_spline_params) + VECSCALE(ARk(vector<Type>(rho_epsilon * rho_vec)), sigma_epsilon)(log_epsilon_f_spline_params);
