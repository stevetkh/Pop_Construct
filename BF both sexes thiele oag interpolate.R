...
compile("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_interpolate.cpp")
dyn.load(dynlib("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_interpolate"))
...
data.loghump.vec.RW <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            ...
                            n_agegroups = bf.idx5$n_ages,
                            fx_idx = bf.idx1$fx_idx,
                            n_fx = bf.idx5$n_fx,
                            census_log_pop_f = data.f, census_log_pop_m = data.m,
                            census_year_idx = match(bf.idx1$interval * floor(as.numeric(colnames(data.f)) / bf.idx1$interval), bf.idx1$periods),
                            ...
                            pop_end = ifelse(apply(data.f, 2, function(i){length(na.omit(i))})-1 > 75, 75, apply(data.f, 2, function(i){length(na.omit(i))})-1),
                            ...
                            df_age = bf5.f.no0.smooth$age2 + 1, dm_age = bf5.m.no0.smooth$age2 + 1,
...
system.time(thiele.f.loghump.oag.RW.ori.interpolate <- fit_tmb(input.thiele.loghump.oag.vec.RW,inner_verbose=TRUE, random = c("log_basepop_f","log_basepop_m",
                                                                                                                      "log_fx",
                                                                                                                      "gx_f","gx_m",
                                                                                                                      "tp_params",
                                                                                                                      "log_phi_f", "log_phi_m",
...
DLL="ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_interpolate",
stepmin = 1e-10, stepmax = 1
)
) 

loghump.models.list <- list("Thiele RW" = thiele.f.loghump.oag.RW.ori.interpolate)
...
thiele.var.sim <-  apply(var.sample(thiele.f.loghump.oag.RW.ori.interpolate, 1000), 1, thiele.f.loghump.oag.RW.ori.interpolate$obj$report)
...
q4515.func <- function(x){
  as_tibble(apply(x$mode$mx_mat_f[15:59 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
    #as_tibble(apply(x$mode$mx_mat_f[4:12,],2,function(x){1-exp(sum(-5*x))})) %>%
    mutate(sex="female", year = bf.idx5$periods) %>%
    bind_rows(
      as_tibble(apply(x$mode$mx_mat_m[15:59 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
...
q4515.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_f[15:59 + 1,],2,function(x){1-prod((1-0.5*x)/(1+0.5*x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "female"),
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_m[15:59 + 1,],2,function(x){1-prod((1-0.5*x)/(1+0.5*x))})}), 1, quantile, c(0.025, 0.975))))%>% 
...
#plot 5q0 estimates####
q50.func <- function(x){
  as_tibble(apply(x$mode$mx_mat_f[0:4 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
    mutate(sex="female", year = bf.idx5$periods) %>%
    bind_rows(
      as_tibble(apply(x$mode$mx_mat_m[0:4 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
        mutate(sex="male", year = bf.idx5$periods)
    )
}
...
q50.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_f[0:4 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "female"),
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_m[0:4 + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "male")
)
...
thiele.loghump <- lapply(loghump.models.list, function(y){y$mode$mx_mat_expand_f %>%
    `colnames<-`(bf.idx1$periods) %>%  
    `rownames<-`(1:nrow(y$mode$mx_mat_expand_f) - 1) %>%
    reshape2::melt()}) %>% 
  map2(names(.), ~ add_column(.x, model = rep(.y, nrow(.x)))) %>% 
  bind_rows() %>%
  ...
  bind_rows(
    lapply(loghump.models.list, function(y){y$mode$mx_mat_expand_m %>%
        `colnames<-`(bf.idx1$periods) %>%  
        `rownames<-`(1:nrow(y$mode$mx_mat_expand_f) - 1) %>%
        reshape2::melt()}) %>% 
      map2(names(.), ~ add_column(.x, model = rep(.y, nrow(.x)))) %>% 
...
mx.var.df.f <- as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$mx_mat_expand_f}), 1, quantile, c(0.025, 0.975)))) %>%
  mutate(sex = "female",
         age5 = rep(0:open.age, bf.idx1$n_periods),
         period5 = rep(bf.idx1$periods, each = open.age+1))

mx.var.df.m <- as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$mx_mat_expand_m}), 1, quantile, c(0.025, 0.975)))) %>%
  mutate(sex = "male",
         age5 = rep(0:open.age, bf.idx1$n_periods),
         period5 = rep(bf.idx1$periods, each = open.age+1))
...
pop.df <- lapply(loghump.models.list, get.pop) %>% 
  ...
  bind_rows(
    ...
    bind_rows(ddharm_bf_census_f_oag %>% mutate(sex = "female"),
      ...
      pivot_longer(!aggr.age & !sex) %>% mutate(year=as.numeric(name), name=NULL, model="UNPD Census smoothed", age = aggr.age, aggr.age=NULL),
