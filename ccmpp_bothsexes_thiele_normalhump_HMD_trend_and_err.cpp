...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(log_dispersion_br); //1 = male, 2 = female
  ...
  DATA_SPARSE_MATRIX(RW2_PREC);
  DATA_SPARSE_MATRIX(RW2_null);
  ...
  DATA_INTEGER(hump_arima_initial);
  ...
  SparseMatrix<Type> QQ_tp_br_age0 = lambda_tp_br_age0 * penal_tp + 4 * penal_tp_0 + null_penal_tp;
  nll += GMRF(QQ_tp_br_age0)(tp_params_br_age0 - tp_slope_br_age0 * tp_mean);
  ...
  SparseMatrix<Type> QQ_tp_br_age1 = lambda_tp_br_age1 * penal_tp + 4 * penal_tp_0 + null_penal_tp;
  nll += GMRF(QQ_tp_br_age1)(tp_params_br_age1 - tp_slope_br_age1 * tp_mean);
  ...
  SparseMatrix<Type> QQ_tp_br_age5 = lambda_tp_br_age5 * penal_tp + 4 * penal_tp_0 + null_penal_tp;
  nll += GMRF(QQ_tp_br_age5)(tp_params_br_age5 - tp_slope_br_age5 * tp_mean);
  ...
  SparseMatrix<Type> QQ_tp_br_age10 = lambda_tp_br_age10 * penal_tp + 4 * penal_tp_0 + null_penal_tp;
  nll += GMRF(QQ_tp_br_age10)(tp_params_br_age10 - tp_slope_br_age10 * tp_mean);
  ...
  nll -= dnorm(sigma_RW2_hump(0), Type(0), Type(0.05), true);
  nll -= dnorm(sigma_RW2_hump(1), Type(0), Type(0.05), true);
  nll -= dnorm(sigma_RW2_hump(2), Type(0), Type(0.05), true);
  nll -= -Type(0.5) * log_prec_all_RW2_hump.sum();

  // SparseMatrix<Type> lambda_PREC = lambda_RW2_hump(0) * RW2_PREC + RW2_null;

  Type rho_lambda = invlogit(logit_rho_lambda);
  nll -= AR1_rho_prior_base0(rho_lambda, theta_rho_hump);
  nll -= Type(2.0) * log(rho_lambda) - logit_rho_lambda;
  
  Type rho_delta = invlogit(logit_rho_delta);
  nll -= AR1_rho_prior_base0(rho_delta, theta_rho_hump);
  nll -= Type(2.0) * log(rho_delta) - logit_rho_delta;

  Type rho_epsilon = invlogit(logit_rho_epsilon);
  nll -= AR1_rho_prior_base0(rho_epsilon, theta_rho_hump);
  nll -= Type(2.0) * log(rho_epsilon) - logit_rho_epsilon;

  nll -= dnorm(log_V_factor_mean, Type(0.0), Type(5.0), true);
  nll -= dnorm(log_V_factor_slope, Type(0.0), Type(0.5), true);
  nll -= dnorm(log_V_factor, Type(0.0), Type(0.5), true);
  Type V_factor_mean = exp(log_V_factor_mean);
  Type V_factor_slope = exp(log_V_factor_slope);
  Type V_factor = exp(log_V_factor);

  MVNORM_t<Type> MVN_mean_mf(V_factor_mean * V_mf_mean);
  MVNORM_t<Type> MVN_slope_mf(V_factor_slope * V_mf_slope);
  MVNORM_t<Type> MVN_err_mf(V_factor * V_mf);
  ...
  if(hump_arima_initial){
	nll -= dnorm(log_spline_hump_m(0, 0), log_spline_prior_means_m(3), Type(0.5), true);
    nll -= dnorm(log_spline_hump_f(0, 0), log_spline_prior_means_f(3), Type(0.5), true);    
    nll -= dnorm(log_spline_hump_m(0, 1), log_spline_prior_means_m(4), Type(0.5), true);
    nll -= dnorm(log_spline_hump_f(0, 1), log_spline_prior_means_f(4), Type(0.5), true);
    nll -= dnorm(log_spline_hump_m(0, 2), log_spline_prior_means_m(5), Type(0.2), true);
    nll -= dnorm(log_spline_hump_f(0, 2), log_spline_prior_means_f(5), Type(0.2), true);
  }

  // nll += GMRF(lambda_PREC)(vector<Type>(log_spline_hump_m.col(0)) - log_spline_prior_means_m(3)) + GMRF(lambda_PREC)(vector<Type>(log_spline_hump_f.col(0)) - log_spline_prior_means_f(3));
  nll += SCALE(AR1(rho_lambda), sigma_RW2_hump(0))(diff(vector<Type>(log_spline_hump_m.col(0)))) + SCALE(AR1(rho_lambda), sigma_RW2_hump(0))(diff(vector<Type>(log_spline_hump_f.col(0))));
  nll += SCALE(AR1(rho_delta), sigma_RW2_hump(1))(diff(vector<Type>(log_spline_hump_m.col(1)))) + SCALE(AR1(rho_delta), sigma_RW2_hump(1))(diff(vector<Type>(log_spline_hump_f.col(1))));
  nll += SCALE(AR1(rho_epsilon), sigma_RW2_hump(2))(diff(vector<Type>(log_spline_hump_m.col(2)))) + SCALE(AR1(rho_epsilon), sigma_RW2_hump(2))(diff(vector<Type>(log_spline_hump_f.col(2))));

  array<Type> temp_arr_m(D_err.rows(), C_dim);
  array<Type> temp_arr_f(D_err.rows(), C_dim);
  temp_arr_m = D_err * log_spline_err_m;
  temp_arr_f = D_err * log_spline_err_f;
 
   for(int p = 0; p < C_dim; p++){
      temp_arr_m.col(p) += log_spline_means_m(p) + log_spline_slopes_m(p) * slope_ind;
      temp_arr_f.col(p) += log_spline_means_f(p) + log_spline_slopes_f(p) * slope_ind;
    }
   
   matrix<Type> temp_mat_m_hump(D_time.rows(), 3);
   matrix<Type> temp_mat_f_hump(D_time.rows(), 3);
   temp_mat_m_hump = D_time * log_spline_hump_m;
   temp_mat_f_hump = D_time * log_spline_hump_f;   
   
   matrix<Type> log_par_m(D_time.rows(), C_dim + 3);
   matrix<Type> log_par_f(D_time.rows(), C_dim + 3);
   log_par_m.col(0) = temp_arr_m.col(0);
   log_par_m.col(1) = temp_arr_m.col(1);
   log_par_m.col(2) = temp_arr_m.col(2);
   log_par_m.col(3) = temp_mat_m_hump.col(0);
   log_par_m.col(4) = temp_mat_m_hump.col(1);
   log_par_m.col(5) = temp_mat_m_hump.col(2);
   log_par_m.col(6) = temp_arr_m.col(3);
   log_par_m.col(7) = temp_arr_m.col(4);
   
   log_par_f.col(0) = temp_arr_f.col(0);
   log_par_f.col(1) = temp_arr_f.col(1);
   log_par_f.col(2) = temp_arr_f.col(2);
   log_par_f.col(3) = temp_mat_f_hump.col(0);
   log_par_f.col(4) = temp_mat_f_hump.col(1);
   log_par_f.col(5) = temp_mat_f_hump.col(2);
   log_par_f.col(6) = temp_arr_f.col(3);
   log_par_f.col(7) = temp_arr_f.col(4);
   
   matrix<Type> par_m = exp(log_par_m.array());
   matrix<Type> par_f = exp(log_par_f.array());

   vector<Type> phi_m = par_m.col(0);
   vector<Type> disp_m = par_m.col(1);
   vector<Type> psi_m = par_m.col(2);
   vector<Type> lambda_m = par_m.col(3);
   vector<Type> delta_m = par_m.col(4);
   vector<Type> epsilon_m = par_m.col(5);
   vector<Type> A_m = par_m.col(6);
   vector<Type> B_m = par_m.col(7);
   
   vector<Type> phi_f = par_f.col(0);
   vector<Type> disp_f = par_f.col(1);
   vector<Type> psi_f = par_f.col(2);
   vector<Type> lambda_f = par_f.col(3);
   vector<Type> delta_f = par_f.col(4);
   vector<Type> epsilon_f = par_f.col(5);
   vector<Type> A_f = par_f.col(6);
   vector<Type> B_f = par_f.col(7);
  ...
  if(loghump) {
      for(int i = 0; i < phi_f.size(); i++){
        mx_mat_f.col(i) = pow(phi_f(i), vector<Type> (pow(thiele_age + disp_f(i), psi_f(i)))) + lambda_f(i)*exp(-delta_f(i)*((log(thiele_age) - log(epsilon_f(i)))*(log(thiele_age) - log(epsilon_f(i))))) + A_f(i)*exp(B_f(i)*(thiele_age - Type(65.0)));
        mx_mat_m.col(i) = pow(phi_m(i), vector<Type> (pow(thiele_age + disp_m(i), psi_m(i)))) + lambda_m(i)*exp(-delta_m(i)*((log(thiele_age) - log(epsilon_m(i)))*(log(thiele_age) - log(epsilon_m(i))))) + A_m(i)*exp(B_m(i)*(thiele_age - Type(65.0)));
      ...
      for(int i = 0; i < phi_f.size(); i++){
        mx_mat_f.col(i) = pow(phi_f(i), vector<Type> (pow(thiele_age + disp_f(i), psi_f(i)))) + lambda_f(i)*exp(-delta_f(i)*((thiele_age - epsilon_f(i))*(thiele_age - epsilon_f(i)))) + A_f(i)*exp(B_f(i)*(thiele_age - Type(65.0)));
        mx_mat_m.col(i) = pow(phi_m(i), vector<Type> (pow(thiele_age + disp_m(i), psi_m(i)))) + lambda_m(i)*exp(-delta_m(i)*((thiele_age - epsilon_m(i))*(thiele_age - epsilon_m(i)))) + A_m(i)*exp(B_m(i)*(thiele_age - Type(65.0)));
        log_v5q0_m(i) = log(Type(1.0) - vector<Type>(Type(1.0) - vector<Type>(mx_mat_m.col(i).head(5)) / (Type(1.0) + Type(0.5) * vector<Type>(mx_mat_m.col(i).head(5)))).prod());
        log_v5q0_f(i) = log(Type(1.0) - vector<Type>(Type(1.0) - vector<Type>(mx_mat_f.col(i).head(5)) / (Type(1.0) + Type(0.5) * vector<Type>(mx_mat_f.col(i).head(5)))).prod());  
      } 
  }
  ...
  nll -= dnorm(log_dispersion_br, Type(0.0), Type(5.0), 1).sum();
  ...
  varf_br = muf_br * (1 + muf_br / exp(log_dispersion_br(1)));
  varm_br = mum_br * (1 + mum_br / exp(log_dispersion_br(0)));
