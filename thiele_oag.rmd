...
__Thiele Normal Hump__
```{r, echo=FALSE}
system.time(thiele.f.oag <- fit_tmb(input.thiele.oag.vec,inner_verbose=FALSE, outer_verbose=FALSE, random = c("log_basepop_f", "log_basepop_m",
                                                                                        "log_fx",
                                                                                        "gx_f","gx_m",
                                                                                        "tp_params",
...
thiele.f.oag$message
```

__Thiele log-Normal Hump__
```{r, echo=FALSE}
system.time(thiele.f.loghump.oag <- fit_tmb(input.thiele.loghump.oag.vec,inner_verbose=FALSE, outer_verbose=FALSE, random = c("log_basepop_f", "log_basepop_m",
                                                                                        "log_fx",
                                                                                        "gx_f","gx_m",
                                                                                        "tp_params",
                                                                                        "log_phi_innov_f", "log_phi_innov_m",
...
thiele.f.loghump.oag$message
```

___Thiele log-Normal Hump RW___
```{r, echo=FALSE}
system.time(thiele.f.loghump.oag.RW.ori <- fit_tmb(input.thiele.loghump.oag.vec.RW,inner_verbose=FALSE, outer_verbose=FALSE, random = c("log_basepop_f","log_basepop_m",
                                                                                                              "log_fx",
                                                                                                              "gx_f","gx_m",
                                                                                                              "tp_params",
                                                                                                              "log_phi_f", "log_phi_m",
...
data.vec.re <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                 log_fx_mean = log_fx_mean,
                 srb = rep(1.05, bf.idx5$n_periods),
                 interval = bf.idx5$interval,
                 n_periods = bf.idx5$n_periods,
                 ...
                 pop_start = rep(2, ncol(data.f)), 
                 pop_end = rep(75/5, ncol(data.f)),
                 #pop_end = ifelse(apply(data.f, 2, function(i){length(na.omit(i))}) > 75/5 + 1, 75/5, apply(data.f, 2, function(i){length(na.omit(i))})-1),
...
data.loghump.vec.re <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                         log_fx_mean = log_fx_mean,
                         srb = rep(1.05, bf.idx5$n_periods),
                         interval = bf.idx5$interval,
                         n_periods = bf.idx5$n_periods,
                         ...
                         census_log_pop_f = data.f, census_log_pop_m = data.m,
                         census_year_idx = match(bf.idx5$interval * floor(as.numeric(colnames(data.f)) / bf.idx5$interval), bf.idx5$periods),
                         census_year_grow_idx = as.numeric(colnames(data.f)) - bf.idx5$interval * floor(as.numeric(colnames(data.f)) /
                                                                                                          bf.idx5$interval),
                         ...
                         Ef = bf5.f.no0.smooth$pyears2, Em = bf5.m.no0.smooth$pyears2,
                         df_age = match(bf5.f.no0.smooth$age5+2, thiele_age), dm_age = match(bf5.m.no0.smooth$age5+2, thiele_age),
                         df_time = match(bf5.f.no0.smooth$period5, levels(bf5.f.no0.smooth$period5)), dm_time = match(bf5.m.no0.smooth$period5,
                                                                                                                levels(bf5.m.no0.smooth$period5)),
                         df_tp = c(bf5.f.no0.smooth$tips)-1, dm_tp = c(bf5.m.no0.smooth$tips)-1,
...
data.loghump.vec.RW.re <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            log_fx_mean = log_fx_mean,
                            srb = rep(1.05, bf.idx5$n_periods),
                            interval = bf.idx5$interval,
                            n_periods = bf.idx5$n_periods,
...
input.thiele.oag.vec.re <- list(data = data.vec.re, par_init = par.vec, model = "ccmpp_vr_tmb")
input.thiele.loghump.oag.vec.re <- list(data = data.loghump.vec.re, par_init = par.vec, model = "ccmpp_vr_tmb")
input.thiele.loghump.oag.vec.RW.re <- list(data = data.loghump.vec.RW.re, par_init = par.vec, model = "ccmpp_vr_tmb")
```

__Thiele Normal Hump (Pop 5-9 to 70-74, DHS 15-19 to 45-49)__
```{r, echo=FALSE}
system.time(thiele.f.oag.re <- fit_tmb(input.thiele.oag.vec.re,inner_verbose=FALSE, outer_verbose=FALSE,  random = c("log_basepop_f", "log_basepop_m",
                                                                                        "log_fx",
                                                                                        "gx_f","gx_m",
                                                                                        "tp_params",
                                                                                        "log_phi_innov_f", "log_phi_innov_m",
...
thiele.f.oag.re$message
```

__Thiele log-Normal Hump (Pop 5-9 to 70-74, DHS 15-19 to 45-49)__
```{r, echo=FALSE}
system.time(thiele.f.loghump.oag.re <- fit_tmb(input.thiele.loghump.oag.vec.re,inner_verbose=FALSE, outer_verbose=FALSE, random = c("log_basepop_f", "log_basepop_m",
                                                                                        "log_fx",
                                                                                        "gx_f","gx_m",
                                                                                        "tp_params",
                                                                                        "log_phi_innov_f", "log_phi_innov_m",
...
thiele.f.loghump.oag.re$message
```

___Thiele log-Normal Hump RW (Pop 5-9 to 70-74, DHS 15-19 to 45-49)___
```{r, echo=FALSE}
system.time(thiele.f.loghump.oag.RW.ori.re <- fit_tmb(input.thiele.loghump.oag.vec.RW.re,inner_verbose=FALSE, outer_verbose=FALSE,random = c("log_basepop_f","log_basepop_m",
                                                                                                              "log_fx",
                                                                                                              "gx_f","gx_m",
                                                                                                              "tp_params",
                                                                                                              "log_phi_f", "log_phi_m",
...
thiele.f.loghump.oag.RW.ori.re$message
```



```{r, include=FALSE}
models.list <- list("Thiele" = thiele.f.oag, "Thiele Restricted Data" = thiele.f.oag.re)
loghump.models.list <- list("Thiele" = thiele.f.loghump.oag, "Thiele RW" = thiele.f.loghump.oag.RW.ori, "Thiele Restricted Data" = thiele.f.loghump.oag.re, "Thiele Restricted Data RW" = thiele.f.loghump.oag.RW.ori.re)
```
...
\newpage
```{r, echo=FALSE, fig.cap = "Estimated parameters"}
...
```{r, echo=FALSE, fig.cap = "Estimated $_{45}q_{15}$"}
...
```{r, echo=FALSE, fig.cap = "Estimated $_{5}q_{0}$"}
q50.func <- function(x){
...
```{r, echo=FALSE, fig.cap = "Mortality Schedules", out.height="0.3\\paperheight"}
...
```{r, echo=FALSE, fig.cap = "Thiele Decomposed", out.height="0.3\\paperheight"}
...
ggplot(filter(thiele.decomp, sex=="female")) + geom_line(aes(x = age5, y = value, col = period5), lwd = 1.2) +
  ...
  ggtitle(paste(country, "Females (Normal hump)")) +
...
ggplot(filter(thiele.decomp, sex=="male")) + geom_line(aes(x = age5, y = value, col = period5), lwd = 1.2) +
  ...
  theme(text = element_text(size=20),
        strip.text = element_text(size=20)) + 
  ggtitle(paste(country, "Males (Normal hump)")) +
  facet_grid(stage~model, scales="free_y", switch = "y")
...
```{r, echo=FALSE, fig.cap="Population", out.height="0.3\\paperheight"}
...
allpop.df %>% filter(!model %in% c("UNPD Census smoothed", "WPP Estimates")) %>%
  ...
  scale_y_continuous(label = function(x) format(x, scientific = TRUE)) +
  ggtitle(country) +
  facet_grid_paginate(age~hump, scale="free", switch="y", nrow = 4, ncol = 2, page=1)
...
allpop.df %>% filter(!model %in% c("UNPD Census smoothed", "WPP Estimates")) %>%
  ...
  scale_y_continuous(label = function(x) format(x, scientific = TRUE)) +
  ggtitle(country) +
  facet_grid_paginate(age~hump, scale="free", switch="y", nrow = 4, ncol = 2, page=2)
...
allpop.df %>% filter(!model %in% c("UNPD Census smoothed", "WPP Estimates")) %>%
  ...
  scale_y_continuous(label = function(x) format(x, scientific = TRUE)) +
  ggtitle(country) +
  facet_grid_paginate(age~hump, scale="free", switch="y", nrow = 4, ncol = 2, page=4)
```
...
```{r, echo=FALSE, fig.cap="Fertility", out.height="0.3\\paperheight"}
