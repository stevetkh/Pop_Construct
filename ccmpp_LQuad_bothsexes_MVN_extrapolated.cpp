...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= dnorm(log_marginal_var_h, Type(0.0), Type(2.0), 1).sum();
  nll -= dnorm(log_marginal_var_k, Type(0.0), Type(2.0), 1).sum();
  ...
  nll -= dnorm(h_params_f, h_constant_f, exp(0.5*log_marginal_var_h(0)), true).sum();
  nll -= dnorm(h_params_m, h_constant_m, exp(0.5*log_marginal_var_h(1)), true).sum();

  //nll += SCALE(AR1(invlogit(logit_rho_h(0))), exp(0.5*log_marginal_var_h(0)))(h_params_f - h_constant_f);
  //nll += SCALE(AR1(invlogit(logit_rho_h(1))), exp(0.5*log_marginal_var_h(1)))(h_params_m - h_constant_m);
