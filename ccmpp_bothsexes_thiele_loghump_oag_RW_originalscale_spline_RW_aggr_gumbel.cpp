...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(log_lambda_gx); //1 = male, 2 = female
  ...
  PARAMETER_VECTOR(log_lambda_phi); //1 = male, 2 = female
  ...
  Type lambda_gx_m = exp(log_lambda_gx(0));
  Type lambda_gx_f = exp(log_lambda_gx(1));
  nll -= gumbel_density(lambda_gx_m, theta_gx);
  nll -= gumbel_density(lambda_gx_f, theta_gx);
  nll -= log_lambda_gx.sum();
  ...
  Type lambda_phi_m = exp(log_lambda_phi(0));
  Type lambda_phi_f = exp(log_lambda_phi(1));
  Type lambda_psi_m = exp(log_lambda_psi(0));
  Type lambda_psi_f = exp(log_lambda_psi(1));
  Type lambda_A_m = exp(log_lambda_A(0));
  Type lambda_A_f = exp(log_lambda_A(1));
  Type lambda_B_m = exp(log_lambda_B(0));
  Type lambda_B_f = exp(log_lambda_B(1));

  nll -= gumbel_density(lambda_phi_m, theta_phi);
  nll -= gumbel_density(lambda_phi_f, theta_phi);
  nll -= gumbel_density(lambda_psi_m, theta_psi);
  nll -= gumbel_density(lambda_psi_f, theta_psi);
  nll -= gumbel_density(lambda_A_m, theta_A);
  nll -= gumbel_density(lambda_A_f, theta_A);
  nll -= gumbel_density(lambda_B_m, theta_B);
  nll -= gumbel_density(lambda_B_f, theta_B);
  nll -= log_lambda_phi.sum();
  nll -= log_lambda_psi.sum();
  nll -= log_lambda_A.sum();
  nll -= log_lambda_B.sum();

  Type lambda_lambda_m = exp(log_lambda_lambda(0));
  Type lambda_lambda_f = exp(log_lambda_lambda(1));
  Type lambda_delta_m = exp(log_lambda_delta(0));
  Type lambda_delta_f = exp(log_lambda_delta(1));
  Type lambda_epsilon_m = exp(log_lambda_epsilon(0));
  Type lambda_epsilon_f = exp(log_lambda_epsilon(1));

  nll -= gumbel_density(lambda_lambda_m, theta_lambda);
  nll -= gumbel_density(lambda_lambda_f, theta_lambda);
  nll -= gumbel_density(lambda_delta_m, theta_delta);
  nll -= gumbel_density(lambda_delta_f, theta_delta);
  nll -= gumbel_density(lambda_epsilon_m, theta_epsilon);
  nll -= gumbel_density(lambda_epsilon_f, theta_epsilon);
  nll -= log_lambda_lambda.sum();
  ...
  SparseMatrix<Type> QQ_phi_f =  lambda_phi_f * full_penal_time;
  nll += GMRF(QQ_phi_f)(log_phi_f_spline_params);
  SparseMatrix<Type> QQ_psi_f =  lambda_psi_f * full_penal_time;
  nll += GMRF(QQ_psi_f)(log_psi_f_spline_params);
  SparseMatrix<Type> QQ_lambda_f =  lambda_lambda_f * full_penal_time;
  nll += GMRF(QQ_lambda_f)(log_lambda_f_spline_params);
  SparseMatrix<Type> QQ_delta_f =  lambda_delta_f * full_penal_time;
  nll += GMRF(QQ_delta_f)(log_delta_f_spline_params);
  SparseMatrix<Type> QQ_epsilon_f =  lambda_epsilon_f * full_penal_time;
  nll += GMRF(QQ_epsilon_f)(log_epsilon_f_spline_params);
  SparseMatrix<Type> QQ_A_f =  lambda_A_f * full_penal_time;
  nll += GMRF(QQ_A_f)(log_A_f_spline_params);
  SparseMatrix<Type> QQ_B_f =  lambda_B_f * full_penal_time;
  nll += GMRF(QQ_B_f)(log_B_f_spline_params);

  SparseMatrix<Type> QQ_phi_m =  lambda_phi_m * full_penal_time;
  nll += GMRF(QQ_phi_m)(log_phi_m_spline_params);
  SparseMatrix<Type> QQ_psi_m =  lambda_psi_m * full_penal_time;
  nll += GMRF(QQ_psi_m)(log_psi_m_spline_params);
  SparseMatrix<Type> QQ_lambda_m =  lambda_lambda_m * full_penal_time;
  nll += GMRF(QQ_lambda_m)(log_lambda_m_spline_params);
  SparseMatrix<Type> QQ_delta_m =  lambda_delta_m * full_penal_time;
  nll += GMRF(QQ_delta_m)(log_delta_m_spline_params);
  SparseMatrix<Type> QQ_epsilon_m =  lambda_epsilon_m * full_penal_time;
  nll += GMRF(QQ_epsilon_m)(log_epsilon_m_spline_params);
  SparseMatrix<Type> QQ_A_m =  lambda_A_m * full_penal_time;
  nll += GMRF(QQ_A_m)(log_A_m_spline_params);
  SparseMatrix<Type> QQ_B_m =  lambda_B_m * full_penal_time;
  ...
  SparseMatrix<Type> QQ_gx_f = lambda_gx_f * full_penal_gx;
  SparseMatrix<Type> QQ_gx_m = lambda_gx_m * full_penal_gx;
