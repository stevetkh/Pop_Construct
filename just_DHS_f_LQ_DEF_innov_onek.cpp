...
//  nll -= dnorm(log_D_mean, Type(-6.0), Type(30.0), 1);
//  nll -= dnorm(log_E_mean, Type(1.5), Type(30.0), 1);
//  nll -= dnorm(log_F_mean, Type(3.5), Type(30.0), 1);
...
//  Type rho_F = 2.0 * invlogit(logit_rho_F) - 1.0;
  ...
  nll -= dnorm(k_param_f, Type(0.0), Type(1.0), 1).sum();
  ...
  nll -= dnorm(log_D, Type(-6.0), Type(5.0), 1);  
  nll -= dnorm(log_E, Type(3.0), Type(5.0), 1);
  nll -= dnorm(log_F, Type(3.5), Type(5.0), 1);
  ...
  log_hump = DEF_log_age - log_F;
  log_hump *= DEF_log_age - log_F;
  log_hump *= -exp(log_E);
  
  //likelihood for DHS data
  vector<Type> muf, varf;
  muf = exp(LQ_baseline_mx_DX_f + h_DX_f*h_params_f + h2_DX_f*h2_params_f + k_DX_f_vec*k_params_f + tp_DX_f*tp_params)*Ef; 
  muf += exp((log_D + log_hump) + tp_DX_f*tp_params)*Ef;
