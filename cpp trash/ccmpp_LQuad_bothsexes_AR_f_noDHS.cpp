...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= dnorm(logit_rho_h, Type(0.0), Type(5.0), 1);
  nll -= dnorm(logit_rho_k, Type(0.0), Type(5.0), 1);
  ...
  nll += SCALE(AR1(invlogit(logit_rho_k)), sigma_k)(k_params_f);
  ...
  log_mx_mat_f.row(0) = h_params_f - log(1 - 0.5 * exp(h_params_f));
