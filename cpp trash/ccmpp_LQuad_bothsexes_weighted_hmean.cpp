...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= dnorm(logit_rho_g_x, Type(0.0), Type(1.0), 1).sum();
  nll -= dnorm(logit_rho_g_t, Type(0.0), Type(1.0), 1).sum();
  ...
  vector<Type> h_params_iid_f(h_params_f.size());
  vector<Type> h_params_iid_m(h_params_m.size());
  h_params_iid_f(0) = h_params_f(0);
  h_params_iid_m(0) = h_params_m(0);
  for(int i = 1; i < h_params_f.size(); i++){
  h_params_iid_f(i) = h_params_f(i) - invlogit(logit_rho_h(0)) * h_params_f(i-1);
  h_params_iid_m(i) = h_params_m(i) - invlogit(logit_rho_h(1)) * h_params_m(i-1);
  }

  vector<Type> h_iid_mean_f = (1 - invlogit(logit_rho_h(0))) * h_mean_f;
  vector<Type> h_iid_mean_m = (1 - invlogit(logit_rho_h(1))) * h_mean_m;  
  h_iid_mean_f(0) += invlogit(logit_rho_h(0)) * h_mean_f(0);
  h_iid_mean_m(0) += invlogit(logit_rho_h(1)) * h_mean_m(0);  

  h_params_iid_f -= h_iid_mean_f;
  h_params_iid_m -= h_iid_mean_m;

  h_params_iid_f *= 1/(sqrt(1 - invlogit(logit_rho_h(0)) * invlogit(logit_rho_h(0))));
  h_params_iid_m *= 1/(sqrt(1 - invlogit(logit_rho_h(1)) * invlogit(logit_rho_h(1)))); 
  h_params_iid_f(0) *= sqrt(1 - invlogit(logit_rho_h(0)) * invlogit(logit_rho_h(0)));
  h_params_iid_m(0) *= sqrt(1 - invlogit(logit_rho_h(1)) * invlogit(logit_rho_h(1)));
   
  nll -= dnorm(h_params_iid_f, Type(0.0), exp(0.5*log_marginal_var_h(0)), 1).sum();
  nll -= dnorm(h_params_iid_m, Type(0.0), exp(0.5*log_marginal_var_h(1)), 1).sum();
