...
Type ccmpp_vr_tmb(objective_function<Type>* obj)
{
  ...
  PARAMETER_VECTOR(sx_params);
  nll -= dnorm(sx_params, sx_params_mean, sigma_sx, true).sum();
  // matrix<Type> logit_sx(basepop.size() + 1, n_periods); 
  // logit_sx = logit_baseline_sx*sx_params.transpose();
  // matrix<Type> sx_mat(invlogit(logit_sx));
  
  vector<Type> logit_sx(logit_baseline_sx.size()*n_periods);
  for(int i = 0; i < n_periods; i++) {
  logit_sx.segment(i*logit_baseline_sx.size(),logit_baseline_sx.size()) = logit_baseline_sx*sx_params(i);
  }
  ...
  for(int i = 1; i < census_year_idx.size(); i++) {
  ...
  vector<Type> var;  
  var = period_deaths * (1 + period_deaths/exp(log_dispersion));

  // likelihood for deaths
  nll -= dnbinom2(deaths_obs_v, period_deaths, var, 1).sum();
