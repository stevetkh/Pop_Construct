...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SPARSE_MATRIX(LQ_baseline_0);
  DATA_SPARSE_MATRIX(tp_DX_f_0);
  ...
  vector<Type> k_innov_f;
  k_innov_f = diff(k_params_f);

  nll += SCALE(AR1(invlogit(logit_rho_k)), sigma_k)(k_innov_f);
  nll -= dnorm(k_params_f(0), Type(0.0), Type(5.0), 1);
