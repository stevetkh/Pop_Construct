...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(log_marginal_var_h);
  PARAMETER(log_marginal_var_k);
  ...
  nll -= dnorm(log_marginal_var_h, Type(0.0), Type(5.0), 1);
  nll -= dnorm(log_marginal_var_k, Type(0.0), Type(5.0), 1);

  nll -= dnorm(logit_rho_h, Type(0.0), Type(1.0), 1);
  nll -= dnorm(logit_rho_k, Type(0.0), Type(1.0), 1);
  ...
  nll += SCALE(AR1(invlogit(logit_rho_h)), exp(0.5*log_marginal_var_h))(h_params_f - h_constant_f);

  nll += SCALE(AR1(invlogit(logit_rho_k)), exp(0.5*log_marginal_var_k))(k_params_f - k_constant_f);
