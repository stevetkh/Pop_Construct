...
Type objective_function<Type>::operator() ()
{
  ...
  SparseMatrix<Type> QQ_h_f = exp(log_lambda_h_f)*penal_mx + exp(-2*log_lambda_h_0_inflated_sd_f)*penal_mx_0;
  nll += GMRF(QQ_h_f)(h_params_f);
  SparseMatrix<Type> QQ_k_f = exp(log_lambda_k_f)*penal_mx + exp(-2*log_lambda_k_0_inflated_sd_f)*penal_mx_0;
  nll += GMRF(QQ_k_f)(k_params_f);
  SparseMatrix<Type> QQ_h_m = exp(log_lambda_h_m)*penal_mx + exp(-2*log_lambda_h_0_inflated_sd_m)*penal_mx_0;
  nll += GMRF(QQ_h_m)(h_params_m);
  SparseMatrix<Type> QQ_k_m = exp(log_lambda_k_m)*penal_mx + exp(-2*log_lambda_k_0_inflated_sd_m)*penal_mx_0;
  nll += GMRF(QQ_k_m)(k_params_m);
