...
pop.f.oag <- filter(pop, Sex=="female") %>% select(-Sex) %>%
  ...
  summarise_at(vars(-aggr.age), DemoTools::graduate, Age = .$aggr.age, method="mono") %>%
  mutate(age = interval * (0:open.age %/% interval)) %>%
  group_by(age) %>%
  summarise_all(sum) 
...
___Thiele log-Normal Hump RW___
```{r, echo=FALSE}
...
\newpage
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Estimated $_{45}q_{15}$"}
...
gbd.bf.q4515 <- filter(gbd.q4515, location_name == country, year_id %in% 1960:2020, sex_name!="both") %>%
  select(c(4,7,12))
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Estimated $_{5}q_{0}$"}
q50.func <- function(x){
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Mortality Schedules", out.height="0.3\\paperheight"}
