...
template <typename Type>
Type d_gamma(const Type& gamma, const Type& rho, const Type& n)
{
  Type ll = 0.0;
  Type denom = 0.0;
  
  denom += (Type(1.0) + Type(3.0) * rho) * (Type(1.0) - rho) * (Type(1.0) - rho);
  ll += (gamma - log(gamma) - Type(1.0)) * n + (Type(1.0) - sqrt(gamma)) * (Type(1.0) - sqrt(gamma)) * Type(2.0) * rho * rho * (Type(3.0) - rho) / denom;
  return sqrt(ll);
}

template <typename Type>
Type logdd_gamma(const Type& gamma, const Type& rho, const Type& n)
{
  Type ll = 0.0;
  Type chunk = 0.0;
  Type denom = 0.0;
...
template <typename Type>
matrix<Type> AR2_innov_prec(const Type& rho, int n)
{
  matrix<Type> I(n, n);
  Type tau2 = 0.0;
  Type det_init = 0.0;
...
template <typename Type>
matrix<Type> AR2_T(const Type& rho, int n)
{
  matrix<Type> I(n, n);
  I.setZero();
...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(theta_gamma_innov);
  DATA_INTEGER(hiv_var_cut);
 
  PARAMETER(log_gamma_lambda);
  PARAMETER(log_gamma_delta);
  PARAMETER(log_gamma_epsilon);
  ...
  int no_basis_time_int = log_phi_f_spline_params.size();
  ...
  Type gamma_lambda = exp(log_gamma_lambda);
  Type gamma_delta = exp(log_gamma_delta);
  Type gamma_epsilon = exp(log_gamma_epsilon);
   
  Type d_gamma_lambda = d_gamma_innov(gamma_lambda, no_basis_time - Type(hiv_var_cut));
  Type d_gamma_delta= d_gamma_innov(gamma_delta, no_basis_time - Type(hiv_var_cut));
  Type d_gamma_epsilon = d_gamma_innov(gamma_epsilon, no_basis_time - Type(hiv_var_cut));

  nll -= dexp(d_gamma_lambda, theta_gamma_innov, 1);
  nll -= dexp(d_gamma_delta, theta_gamma_innov, 1);
  nll -= dexp(d_gamma_epsilon, theta_gamma_innov, 1);

  nll -= logdd_gamma_innov(gamma_lambda, no_basis_time- Type(hiv_var_cut));
  nll -= logdd_gamma_innov(gamma_delta, no_basis_time- Type(hiv_var_cut));
  nll -= logdd_gamma_innov(gamma_epsilon, no_basis_time- Type(hiv_var_cut));

  nll -= log_gamma_lambda;
  nll -= log_gamma_delta;
  nll -= log_gamma_epsilon;
  ...
  matrix<Type> lambda_AR2_innov_prec = AR2_innov_prec(rho_lambda, no_basis_time_int);
  matrix<Type> delta_AR2_innov_prec = AR2_innov_prec(rho_delta, no_basis_time_int);
  matrix<Type> epsilon_AR2_innov_prec = AR2_innov_prec(rho_epsilon, no_basis_time_int);
  matrix<Type> lambda_AR2_T = AR2_T(rho_lambda, no_basis_time_int);
  matrix<Type> delta_AR2_T = AR2_T(rho_delta, no_basis_time_int);
  matrix<Type> epsilon_AR2_T = AR2_T(rho_epsilon, no_basis_time_int);
  
  for(int i = 0; i < hiv_var_cut; i++){
    lambda_AR2_innov_prec(i,i) *= exp(-log_marginal_lambda_lambda);
    delta_AR2_innov_prec(i,i) *= exp(-log_marginal_lambda_delta);
    epsilon_AR2_innov_prec(i,i) *= exp(-log_marginal_lambda_epsilon);
  }

    lambda_AR2_innov_prec(hiv_var_cut, hiv_var_cut) *= exp(-log_marginal_lambda_lambda) / (Type(0.25) * gamma_lambda + Type(0.75));
    delta_AR2_innov_prec(hiv_var_cut, hiv_var_cut) *= exp(-log_marginal_lambda_delta) / (Type(0.25) * gamma_delta + Type(0.75));
    epsilon_AR2_innov_prec(hiv_var_cut, hiv_var_cut) *= exp(-log_marginal_lambda_epsilon) / (Type(0.25) * gamma_epsilon + Type(0.75));

    lambda_AR2_innov_prec(hiv_var_cut + 1, hiv_var_cut + 1) *= exp(-log_marginal_lambda_lambda) / (Type(0.5) * gamma_lambda + Type(0.5));
    delta_AR2_innov_prec(hiv_var_cut + 1, hiv_var_cut + 1) *= exp(-log_marginal_lambda_delta) / (Type(0.5) * gamma_delta + Type(0.5));
    epsilon_AR2_innov_prec(hiv_var_cut + 1, hiv_var_cut + 1) *= exp(-log_marginal_lambda_epsilon) / (Type(0.5) * gamma_epsilon + Type(0.5));

    lambda_AR2_innov_prec(hiv_var_cut + 2, hiv_var_cut + 2) *= exp(-log_marginal_lambda_lambda) / (Type(0.75) * gamma_lambda + Type(0.25));
    delta_AR2_innov_prec(hiv_var_cut + 2, hiv_var_cut + 2) *= exp(-log_marginal_lambda_delta) / (Type(0.75) * gamma_delta + Type(0.25));
    epsilon_AR2_innov_prec(hiv_var_cut + 2, hiv_var_cut + 2) *= exp(-log_marginal_lambda_epsilon) / (Type(0.75) * gamma_epsilon + Type(0.25));

  for(int i = 0; i < no_basis_time_int - hiv_var_cut - 3; i++){
    lambda_AR2_innov_prec(hiv_var_cut + 3 + i, hiv_var_cut + 3 + i) *= exp(-log_marginal_lambda_lambda) / gamma_lambda;
    delta_AR2_innov_prec(hiv_var_cut + 3 + i, hiv_var_cut + 3 + i) *= exp(-log_marginal_lambda_delta) / gamma_delta;
    epsilon_AR2_innov_prec(hiv_var_cut + 3 + i, hiv_var_cut + 3 + i) *= exp(-log_marginal_lambda_epsilon) / gamma_epsilon;
  }
  ...
  SparseMatrix<Type> QQ_lambda = asSparseMatrix( matrix<Type> (lambda_AR2_T.transpose() * lambda_AR2_innov_prec * lambda_AR2_T) );
  SparseMatrix<Type> QQ_delta = asSparseMatrix( matrix<Type> (delta_AR2_T.transpose() * delta_AR2_innov_prec * delta_AR2_T) );
  SparseMatrix<Type> QQ_epsilon = asSparseMatrix( matrix<Type> (epsilon_AR2_T.transpose() * epsilon_AR2_innov_prec * epsilon_AR2_T) );
  
  nll += GMRF(QQ_lambda)(log_lambda_m_spline_params) + GMRF(QQ_lambda)(log_lambda_f_spline_params) ;
  nll += GMRF(QQ_delta)(log_delta_m_spline_params) + GMRF(QQ_delta)(log_delta_f_spline_params);
  nll += GMRF(QQ_epsilon)(log_epsilon_m_spline_params) + GMRF(QQ_epsilon)(log_epsilon_f_spline_params);
