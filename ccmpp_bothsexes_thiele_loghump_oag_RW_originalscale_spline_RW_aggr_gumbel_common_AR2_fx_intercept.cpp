...
Type objective_function<Type>::operator() ()
{
  ...
  nll -= dnorm(log_fx_intercept, Type(0.0), Type(0.0486), true);
  nll += GMRF(QQ_fx)(log_fx_spline_params);
  vector<Type> fx(exp(log_fx_mean + log_fx_intercept + D_agetime_fert * log_fx_spline_params));
