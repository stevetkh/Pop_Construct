...
template <typename Type>
vector<Type> pacf2phi = (const vector<Type>& pac) 
{
  vector<Type> phi = pac;
  vector<Type> work = pac;
  if (pac.size() > 1)
    for (int j = 1; j < pac.size(); j++) {
      work.segment(0, j) = work.segment(0, j) - phi(j) * phi.segment(0, j).reverse();
      phi.segment(0, j) = work.segment(0, j);
    }
  return phi;
}
