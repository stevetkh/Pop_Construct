...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER_VECTOR(log_phi_f); 
  PARAMETER_VECTOR(log_psi_f);
  PARAMETER_VECTOR(log_lambda_f);
  PARAMETER_VECTOR(log_delta_f);
  PARAMETER_VECTOR(log_epsilon_f);
  PARAMETER_VECTOR(log_A_f);
  PARAMETER_VECTOR(log_B_f);

  PARAMETER_VECTOR(log_phi_m); 
  PARAMETER_VECTOR(log_psi_m);
  PARAMETER_VECTOR(log_lambda_m);
  PARAMETER_VECTOR(log_delta_m);
  PARAMETER_VECTOR(log_epsilon_m);
  PARAMETER_VECTOR(log_A_m);
  PARAMETER_VECTOR(log_B_m);
  ...
  for(int i=0; i < n_periods; i++) {
    ...
    sx_mat_m(0,i) = (Type(1.0) - Type(0.15) * mx_mat_f(0,i)) / (Type(1.0) + Type(0.5) * mx_mat_m(0,i));
  ...
  matrix<Type> mx_mat_expand_m(open_idx, n_periods*interval);
  matrix<Type> mx_mat_expand_f(open_idx, n_periods*interval);
  matrix<Type> sx_mat_expand_m(open_idx + 1, n_periods*interval);
  matrix<Type> sx_mat_expand_f(open_idx + 1, n_periods*interval);

  for(int i=0; i<n_periods; i++){
	for(int j=0; j<interval; j++){
		mx_mat_expand_m.col(interval*i + j) = mx_mat_aggr_m.col(i);
		mx_mat_expand_f.col(interval*i + j) = mx_mat_aggr_f.col(i); 
		sx_mat_expand_m.col(interval*i + j) = sx_mat_m.col(i);
		sx_mat_expand_f.col(interval*i + j) = sx_mat_f.col(i); 
	}
  }
  ...
  for(int i = 0; i < df.size(); i++){
    muf(i) = mx_mat_expand_f(df_age(i)-1, df_time(i)-1) * exp(tp_params(df_tp(i))) * Ef(i);
  }
  for(int i = 0; i < dm.size(); i++){
    mum(i) = mx_mat_expand_m(dm_age(i)-1, dm_time(i)-1) * exp(tp_params(dm_tp(i))) * Em(i);
  }
  ...
  matrix<Type> fx_mat_rowexpand(n_fx*interval, n_periods);
  for(int i=0; i<n_fx; i++){
	for(int j=0; j<interval; j++){
		fx_mat_rowexpand.row(interval*i + j) = fx_mat.row(i);
	}
  }

  matrix<Type> fx_mat_expand(n_fx*interval, n_periods*interval);
  for(int i=0; i<n_periods; i++){
	for(int j=0; j<interval; j++){
		fx_mat_expand.col(interval*i + j) = fx_mat_rowexpand.col(i);
	}
  }


  // prior for gx
  MapMatrixXXt gx_mat_f(gx_f.data(), n_agegroups, n_periods);
  MapMatrixXXt gx_mat_m(gx_m.data(), n_agegroups, n_periods);
  array<Type> gx_f_array(n_agegroups, n_periods);
  array<Type> gx_m_array(n_agegroups, n_periods);
  gx_f_array = gx_mat_f.array();
  gx_m_array = gx_mat_m.array();

  matrix<Type> gx_mat_rowexpand_f((n_agegroups-1)*interval + 1, n_periods);
  matrix<Type> gx_mat_rowexpand_m((n_agegroups-1)*interval + 1, n_periods);
  gx_mat_rowexpand_f.row((n_agegroups-1)*interval) = gx_mat_f.row(n_agegroups - 1);
  gx_mat_rowexpand_m.row((n_agegroups-1)*interval) = gx_mat_m.row(n_agegroups - 1);  

  for(int i=0; i<(n_agegroups-1); i++){
	for(int j=0; j<interval; j++){
		gx_mat_rowexpand_m.row(interval*i + j) = gx_mat_m.row(i);
		gx_mat_rowexpand_f.row(interval*i + j) = gx_mat_f.row(i);
	}
  }

  matrix<Type> gx_mat_expand_f((n_agegroups-1)*interval + 1, n_periods*interval);
  matrix<Type> gx_mat_expand_m((n_agegroups-1)*interval + 1, n_periods*interval);
  for(int i=0; i<n_periods; i++){
	for(int j=0; j<interval; j++){
		gx_mat_expand_m.col(interval*i + j) = gx_mat_rowexpand_m.col(i);
		gx_mat_expand_f.col(interval*i + j) = gx_mat_rowexpand_f.col(i);
	}
  }
  ...
  PopulationProjection<Type> proj(ccmpp<Type>(basepop_f, sx_mat_expand_f, fx_mat_expand, gx_mat_expand_f,
					      srb, Type(1.0), fx_idx-1));
  PopulationProjection_m<Type> proj_m(ccmpp_m<Type>(basepop_m, sx_mat_expand_m, gx_mat_expand_m,
  ...
  if(calc_outputs) {
    ...
    REPORT(fx);
    REPORT(gx_f);
    REPORT(sx_mat_expand_f);
    REPORT(mx_mat_expand_f);
    REPORT(fx_mat_expand);
    REPORT(gx_mat_expand_f);
    ...
    REPORT(mx_mat_m);
    REPORT(gx_m);
    REPORT(sx_mat_expand_m);
    REPORT(mx_mat_expand_m);
    REPORT(gx_mat_expand_m);
