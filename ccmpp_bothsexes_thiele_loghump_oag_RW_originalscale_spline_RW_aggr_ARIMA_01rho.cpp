...
Type objective_function<Type>::operator() ()
{
  ...
  Type rho_lambda_f = invlogit(logit_lambda_slope_rho_f);
  nll -= dnorm(logit_delta_slope_rho_f, Type(0.0), Type(5.0),1);
  Type rho_delta_f = invlogit(logit_delta_slope_rho_f);
  nll -= dnorm(logit_epsilon_slope_rho_f, Type(0.0), Type(5.0),1);
  Type rho_epsilon_f = invlogit(logit_epsilon_slope_rho_f);

  nll -= dnorm(logit_lambda_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_lambda_m = invlogit(logit_lambda_slope_rho_m);
  nll -= dnorm(logit_delta_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_delta_m = invlogit(logit_delta_slope_rho_m);
  nll -= dnorm(logit_epsilon_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_epsilon_m = invlogit(logit_epsilon_slope_rho_m);
