...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_VECTOR(df);
  DATA_VECTOR(Ef);
  DATA_VECTOR(DEF_log_age_f);
  DATA_SPARSE_MATRIX(DEF_time_f);
  ...
  DATA_VECTOR(dm);
  DATA_VECTOR(Em);
  DATA_VECTOR(DEF_log_age_m);
  DATA_SPARSE_MATRIX(DEF_time_m);
  ...
  PARAMETER(log_D_mean_f);
  PARAMETER_VECTOR(log_E_f);
  PARAMETER(log_E_mean_f);
  PARAMETER_VECTOR(log_F_f);
  PARAMETER(log_F_mean_f);

  PARAMETER(logit_rho_D_f);
  PARAMETER(logit_rho_E_f);
  PARAMETER(logit_rho_F_f);

  PARAMETER(log_marginal_prec_D_f);
  PARAMETER(log_marginal_prec_E_f);
  PARAMETER(log_marginal_prec_F_f);
  ...
  PARAMETER_VECTOR(log_D_m);
  PARAMETER(log_D_mean_m);
  PARAMETER_VECTOR(log_E_m);
  PARAMETER(log_E_mean_m);
  PARAMETER_VECTOR(log_F_m);
  PARAMETER(log_F_mean_m);

  PARAMETER(logit_rho_D_m);
  PARAMETER(logit_rho_E_m);
  PARAMETER(logit_rho_F_m);

  PARAMETER(log_marginal_prec_D_m);
  PARAMETER(log_marginal_prec_E_m);
  PARAMETER(log_marginal_prec_F_m);
  ...
  nll -= dlgamma(log_marginal_prec_D_f, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_D_f(exp(-0.5 * log_marginal_prec_D_f));
  nll -= dlgamma(log_marginal_prec_E_f, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_E_f(exp(-0.5 * log_marginal_prec_E_f));
  nll -= dlgamma(log_marginal_prec_F_f, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_F_f(exp(-0.5 * log_marginal_prec_F_f));
  nll -= dnorm(logit_rho_D_f, Type(0.0), Type(10.0), 1);
  nll -= dnorm(logit_rho_E_f, Type(0.0), Type(10.0), 1);
  nll -= dnorm(logit_rho_F_f, Type(0.0), Type(10.0), 1);
  nll -= dnorm(log_D_mean_f, Type(-6.0), Type(2.0), 1);
  nll -= dnorm(log_E_mean_f, Type(1.5), Type(2.0), 1);
  nll -= dnorm(log_F_mean_f, Type(3.5), Type(3.0), 1);

  nll -= dlgamma(log_marginal_prec_D_m, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_D_m(exp(-0.5 * log_marginal_prec_D_m));
  nll -= dlgamma(log_marginal_prec_E_m, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_E_m(exp(-0.5 * log_marginal_prec_E_m));
  nll -= dlgamma(log_marginal_prec_F_m, Type(1.0), Type(1.0 / 5.0), true);
  Type sigma_F_m(exp(-0.5 * log_marginal_prec_F_m));
  nll -= dnorm(logit_rho_D_m, Type(0.0), Type(10.0), 1);
  nll -= dnorm(logit_rho_E_m, Type(0.0), Type(10.0), 1);
  nll -= dnorm(logit_rho_F_m, Type(0.0), Type(10.0), 1);
  nll -= dnorm(log_D_mean_m, Type(-6.0), Type(2.0), 1);
  nll -= dnorm(log_E_mean_m, Type(1.5), Type(2.0), 1);
  nll -= dnorm(log_F_mean_m, Type(3.5), Type(3.0), 1);
  ...
  Type rho_D_f = 2.0 * invlogit(logit_rho_D_f) - 1.0;
  Type rho_E_f = 2.0 * invlogit(logit_rho_E_f) - 1.0;
  Type rho_F_f = 2.0 * invlogit(logit_rho_F_f) - 1.0;
  Type rho_D_m = 2.0 * invlogit(logit_rho_D_m) - 1.0;
  Type rho_E_m = 2.0 * invlogit(logit_rho_E_m) - 1.0;
  Type rho_F_m = 2.0 * invlogit(logit_rho_F_m) - 1.0;
  ...
  nll += SCALE(AR1(rho_D_f), sigma_D_f)(log_D_f - log_D_mean_f);
  nll += SCALE(AR1(rho_E_f), sigma_E_f)(log_E_f - log_E_mean_f);
  nll += SCALE(AR1(rho_F_f), sigma_F_f)(log_F_f - log_F_mean_f);

  nll += SCALE(AR1(rho_D_m), sigma_D_m)(log_D_m - log_D_mean_m);
  nll += SCALE(AR1(rho_E_m), sigma_E_m)(log_E_m - log_E_mean_m);
  nll += SCALE(AR1(rho_F_m), sigma_F_m)(log_F_m - log_F_mean_m);
  ...
  vector<Type> log_hump_f;
  log_hump_f = DEF_log_age_f - DEF_time_f * log_F_f;
  log_hump_f *= DEF_log_age_f - DEF_time_f * log_F_f;
  log_hump_f *= -exp(DEF_time_f * log_E_f);
  
  vector<Type> log_hump_m;
  log_hump_m = DEF_log_age_m - DEF_time_m * log_F_m;
  log_hump_m *= DEF_log_age_m - DEF_time_m * log_F_m;
  log_hump_m *= -exp(DEF_time_m * log_E_m);
  ...
  muf += exp(DEF_time_f * log_D_f + log_hump_f + tp_DX_f*tp_params)*Ef;
  mum += exp(DEF_time_m * log_D_m + log_hump_m + tp_DX_m*tp_params)*Em;
  ...
  matrix<Type> log_DEF_mat_f(LQ_baseline_f.rows() + 1, n_periods);
  for(int i=0; i < n_periods; i++){
  ...
  matrix<Type> log_DEF_mat_m(LQ_baseline_m.rows() + 1, n_periods);
  for(int i=0; i < n_periods; i++){
    log_DEF_mat_m.col(i) = log_D_m(i) - exp(log_E_m(i)) * (DEF_log_age - log_F_m(i)) * (DEF_log_age - log_F_m(i));
  }
  matrix<Type> DEF_mat_m = exp(log_DEF_mat_m.array());
  mx_mat_m += DEF_mat_m;
