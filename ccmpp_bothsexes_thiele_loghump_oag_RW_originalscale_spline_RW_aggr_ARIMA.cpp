...
Type objective_function<Type>::operator() ()
{
  ...
  Type rho_lambda_f = 2.0 * invlogit(logit_lambda_slope_rho_f) - 1.0;
  nll -= dnorm(logit_delta_slope_rho_f, Type(0.0), Type(5.0),1);
  Type rho_delta_f = 2.0 * invlogit(logit_delta_slope_rho_f) - 1.0;
  nll -= dnorm(logit_epsilon_slope_rho_f, Type(0.0), Type(5.0),1);
  Type rho_epsilon_f = 2.0 * invlogit(logit_epsilon_slope_rho_f) - 1.0;

  nll -= dnorm(logit_lambda_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_lambda_m = 2.0 * invlogit(logit_lambda_slope_rho_m) - 1.0;
  nll -= dnorm(logit_delta_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_delta_m = 2.0 * invlogit(logit_delta_slope_rho_m) - 1.0;
  nll -= dnorm(logit_epsilon_slope_rho_m, Type(0.0), Type(5.0),1);
  Type rho_epsilon_m = 2.0 * invlogit(logit_epsilon_slope_rho_m) - 1.0;
