...
Type objective_function<Type>::operator() ()
{
  ...
  h_params_iid_f(i) = h_params_f(i) - rho_h * h_params_f(i-1);
  }

  vector<Type> h_iid_mean_f = (1.0 - rho_h) * h_constant_f;
  h_iid_mean_f(0) += rho_h * h_constant_f(0);

  h_params_iid_f -= h_iid_mean_f;

  h_params_iid_f *= 1/sqrt(1 - rho_h * rho_h);
  h_params_iid_f(0) *= sqrt(1 - rho_h * rho_h);

  nll -= dnorm(h_params_iid_f, Type(0.0), sigma_h, 1).sum();
  nll -= (0.5 - h_params_f.size() / 2) * log(1.0 - rho_h * rho_h);
