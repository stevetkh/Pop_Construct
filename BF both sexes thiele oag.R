...
compile("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_oag.cpp")
dyn.load(dynlib("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_oag"))
...
compile("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_Ad.cpp")
dyn.load(dynlib("C:/Users/ktang3/Desktop/Imperial/Pop_Construct/ccmpp_bothsexes_thiele_loghump_oag_RW_originalscale_Ad"))
...
pop.f.oag <- filter(pop, Sex=="female") %>% select(-Sex) %>%
  mutate(aggr.age = ifelse(age >= open.age, open.age, age)) %>%
  group_by(aggr.age) %>%
  summarise_at(vars(-age), sum, na.rm=T)

pop.m.oag <- filter(pop, Sex=="male") %>% select(-Sex) %>%
  mutate(aggr.age = ifelse(age >= open.age, open.age, age)) %>%
  group_by(aggr.age) %>%
  summarise_at(vars(-age), sum, na.rm=T)
...
thiele.loghump.prior <- function(h, sex) {
  ...
  nlm <- nlminb(start = log(c(exp(log.m0), psi, 2e-3, 0.7, 22, exp(LQ.mx[19])-2e-3*exp(-0.5*(log(92)-log(22))^2), 0.1)), thiele.min, dat = LQ.mx[1:19], control = list(eval.max = 8000, iter.max = 8000, step.min = 1e-10, step.max = 1e-4))
  stopifnot(nlm$convergence ==0)
  cat(nlm$message,"\n")
  return(setNames(exp(nlm$par), c("phi", "psi", "lambda", "delta", "epsilon", "A", "B")))
}
...
data.loghump.vec.RW <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            ...
                            log_phi_hypervar_prec = log(2/0.01),
                            log_psi_hypervar_prec = log(2/0.01),
...
par.vec <- list(log_tau2_logpop_f = c(2,4), log_tau2_logpop_m = c(2,4),
                ...
                log_marginal_prec_A_d = prec.init,
...
system.time(thiele.f.loghump.oag <- fit_tmb(input.thiele.loghump.oag.vec,inner_verbose=TRUE, random = c("log_basepop_f", "log_basepop_m",
                                            ...
                                            map = list(
                                              ...
                                              #log_tau2_logpop_m = factor(c(NA,NA)),
                                              #logit_rho_phi_m = factor(NA), logit_rho_phi_f = factor(NA),
                                              #logit_rho_psi_f = factor(NA), logit_rho_psi_m = factor(NA),
                                              #logit_rho_lambda_f = factor(NA), logit_rho_lambda_m = factor(NA),
                                              #logit_rho_delta_f = factor(NA),logit_rho_delta_m = factor(NA),
                                              #logit_rho_epsilon_f = factor(NA), logit_rho_epsilon_m = factor(NA),
...
system.time(thiele.f.loghump.oag.RW.ori.Ad <- fit_tmb(input.thiele.loghump.oag.vec.RW,inner_verbose=TRUE, random = c("log_basepop_f", "log_basepop_m",
                                                                                                                  "log_fx",
                                                                                                                  "gx_f","gx_m",
                                                                                                                  "tp_params",
                                                                                                                  "log_phi_f", "log_phi_m",
...
system.time(thiele.f.loghump.oag.MVN <- fit_tmb(input.thiele.loghump.oag.vec,inner_verbose=TRUE, random = c("log_basepop_f", "log_basepop_m",
                                                                                                        "log_fx",
                                                                                                        "gx_f","gx_m",
                                                                                                        "tp_params",
                                                                                                        "log_phi_innov_f", "log_phi_innov_m",
...
q4515.df <- lapply(models.list, q4515.func) %>% 
  ...
  bind_rows(
    ...
    apply(apply(thiele.prior.m, 2, function(i) {i[1] * exp(-i[2] * (seq(17, 57, by = 5)-2)) + 
        i[3] * exp(-i[4] * ((seq(17, 57, by = 5) - i[5])^2)) +
        i[6] * exp(i[7] * (seq(17, 57, by = 5)-92))}) %>%
          `colnames<-`(bf.idx5$periods) %>% 
          `rownames<-`(seq(15, 55, by = 5)), 2, function(j){1-prod((1-2.5*j)/(1+2.5*j))}) %>%
...
q50.df <- lapply(models.list, q50.func) %>% 
  ...
  bind_rows(
    ...
    as_tibble(5 * apply(thiele.prior.m, 2, function(i) {i[1] * exp(-i[2] * 0) + i[3] * exp(-i[4] * (2 - i[5])^2) + i[6] * exp(i[7] * (2-92))}) / 
