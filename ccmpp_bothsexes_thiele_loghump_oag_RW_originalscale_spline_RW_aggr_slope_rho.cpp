...
Type objective_function<Type>::operator() ()
{
  ...
  PARAMETER(logit_lambda_slope_rho_f);
  PARAMETER(logit_delta_slope_rho_f);
  PARAMETER(logit_epsilon_slope_rho_f);

  PARAMETER(logit_lambda_slope_rho_m);
  PARAMETER(logit_delta_slope_rho_m);
  PARAMETER(logit_epsilon_slope_rho_m);
  ...
  nll -= dnorm(logit_lambda_slope_rho_f, Type(0.0), Type(5.0),1);
  nll -= dnorm(logit_delta_slope_rho_f, Type(0.0), Type(5.0),1);
  nll -= dnorm(logit_epsilon_slope_rho_f, Type(0.0), Type(5.0),1);

  nll -= dnorm(logit_lambda_slope_rho_m, Type(0.0), Type(5.0),1);
  nll -= dnorm(logit_delta_slope_rho_m, Type(0.0), Type(5.0),1);
  nll -= dnorm(logit_epsilon_slope_rho_m, Type(0.0), Type(5.0),1);

  Type no_basis = log_lambda_f_spline_params.size();
  ...
  nll -= dnorm(log_lambda_f_spline_params(1) - log_lambda_f_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_lambda_f_spline_params).tail(no_basis - 2) - invlogit(logit_lambda_slope_rho_f) * diff(log_lambda_f_spline_params).head(no_basis-2), Type(0.0), sigma_lambda_f, true).sum();

  nll -= dnorm(log_delta_f_spline_params(0), log_delta_mean_f, Type(0.3), true);
  nll -= dnorm(log_delta_f_spline_params(1) - log_delta_f_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_delta_f_spline_params).tail(no_basis - 2) - invlogit(logit_delta_slope_rho_f) * diff(log_delta_f_spline_params).head(no_basis-2), Type(0.0), sigma_delta_f, true).sum();

  nll -= dnorm(log_epsilon_f_spline_params(0), log_epsilon_mean_f, Type(0.5), true);
  nll -= dnorm(log_epsilon_f_spline_params(1) - log_epsilon_f_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_epsilon_f_spline_params).tail(no_basis - 2) - invlogit(logit_epsilon_slope_rho_f) * diff(log_epsilon_f_spline_params).head(no_basis-2), Type(0.0), sigma_epsilon_f, true).sum();
  ...
  nll -= dnorm(log_lambda_m_spline_params(1) - log_lambda_m_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_lambda_m_spline_params).tail(no_basis - 2) - invlogit(logit_lambda_slope_rho_m) * diff(log_lambda_m_spline_params).head(no_basis-2), Type(0.0), sigma_lambda_m, true).sum();

  nll -= dnorm(log_delta_m_spline_params(0), log_delta_mean_m, Type(0.3), true);
  nll -= dnorm(log_delta_m_spline_params(1) - log_delta_m_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_delta_m_spline_params).tail(no_basis - 2) - invlogit(logit_delta_slope_rho_m) * diff(log_delta_m_spline_params).head(no_basis-2), Type(0.0), sigma_delta_m, true).sum();

  nll -= dnorm(log_epsilon_m_spline_params(0), log_epsilon_mean_m, Type(0.5), true);
  nll -= dnorm(log_epsilon_m_spline_params(1) - log_epsilon_m_spline_params(0), Type(0.0), Type(0.05), true);
  nll -= dnorm(diff(log_epsilon_m_spline_params).tail(no_basis - 2) - invlogit(logit_epsilon_slope_rho_m) * diff(log_epsilon_m_spline_params).head(no_basis-2), Type(0.0), sigma_epsilon_m, true).sum();
