...
data.loghump.vec.RW <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            ...
                            penal_age = diag(no.basis) %x% as(crossprod(diff(diag(no.basis))),"sparseMatrix"),
                            penal_time = as(crossprod(diff(diag(no.basis))),"sparseMatrix") %x% diag(no.basis),
                            penal_agetime = as(crossprod(diff(diag(no.basis))%x%diff(diag(no.basis))),"sparseMatrix"),
