...
Type objective_function<Type>::operator() ()
{
  ...
  nll += SCALE(AR1(rho_lambda), sigma_RW2_hump(0))(diff(vector<Type>(log_spline_hump_m.col(0))));
  nll += SCALE(AR1(rho_lambda), sigma_RW2_hump(0))(diff(vector<Type>(log_spline_hump_f.col(0))));
...
//		vector<Type>(census_deaths_mat_m.block(deaths_start(i) - 1, i, deaths_end(i) - deaths_start(i) + 1, 1)), 1).sum();
  ...
  for(int i = 0; i < census_deaths_year_idx_5.size(); i++) {	
    ...
    REPORT(log_spline_means_m);
	REPORT(log_spline_means_f);

    REPORT(log_spline_slopes_m);
	REPORT(log_spline_slopes_f);
	
    REPORT(log_spline_err_m);
	REPORT(log_spline_err_f);
