...
ddharm_bf_census_f_oag <- ddharm_census_f_smoothed %>%
  ...
  mutate(age = interval * (aggr.age %/% interval)) %>%
  group_by(age) %>%
...
bf.idx5<-projection_indices(period_start = 1960,
                            ...
                            interval = interval,
                            n_ages = n_ages,
                            fx_idx = 15 %/% interval + 1,
                            n_fx = 49 %/% interval - 15 %/% interval + 1,
                            n_sexes = 2)
##WPP fx
...
fx_init <-fx_init.singleyear %>% #pad fx at 14 = 0
  as_tibble() %>%
  mutate(combine.age = interval * (15:49 %/% interval)) %>%
  group_by(combine.age) %>%
  summarise_all(function(i){sum(i)/interval}) %>%
  select(-1) %>%
  as.matrix()
...
igme.h.mean.f <- igme.5q0.df %>% filter(Sex=="Female", year %in% (bf.idx5$periods + interval%/%2)) %>% .$child.mort %>% log()
h.mean.f <- wpp.5q0.interpolate %>% filter(Sex=="Female", year %in% (bf.idx5$periods + interval%/%2)) %>% .$q50 %>% log() ##Using WPP 5q0 estimates
...
igme.h.mean.m <- igme.5q0.df %>% filter(Sex=="Male", year %in% (bf.idx5$periods + interval%/%2)) %>% .$child.mort %>% log()
h.mean.m <- wpp.5q0.interpolate %>% filter(Sex=="Male", year %in% (bf.idx5$periods + interval%/%2)) %>% .$q50 %>% log() ##Using WPP 5q0 estimates
#thiele.prior.m <- sapply(h.mean.m, thiele.prior, sex="male")
thiele.loghump.prior.m <- sapply(h.mean.m, thiele.loghump.prior, sex="male")

thiele_age <- seq(0, 95, by = interval) + interval%/%2

##DHS data cohort smoothed
bf5.smooth <- aggr.mat.cohort.0[[country]] %>%
  mutate(age2 =  interval * (agegr %/% interval),
         period2 =  interval * (period %/%  interval)) %>%
  filter(period2 %in% bf.idx5$periods) %>%
  group_by(mm1, tips, DHS, age2, period2) %>%
  summarise_at(vars(pyears, event, pyears2, adjusted), sum) %>%
  ungroup()

bf5.smooth$period2 <- factor(bf5.smooth$period2,levels=bf.idx5$periods)
bf5.smooth$tips <- factor(bf5.smooth$tips,levels=0:14)

dhs.start.age <- 15
dhs.end.age <- 59

bf5.f.no0.smooth <- bf5.smooth %>% filter(mm1=="female", age2 >= dhs.start.age, age2 <= dhs.end.age) %>% arrange(period2,tips,age2)
bf5.m.no0.smooth <- bf5.smooth %>% filter(mm1=="male", age2 >= dhs.start.age, age2 <= dhs.end.age) %>% arrange(period2,tips,age2)

basepop.f <- ifelse(pop.f.oag$`1960`==0, 10, pop.f.oag$'1960')
...
#init_lambda_f <- 0.005; init_lambda_m <- 0.005; 
...
data.loghump.vec.RW <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            ...
                            pop_end = ifelse(apply(data.f, 2, function(i){length(na.omit(i))})-1 > 75 %/% interval, 75 %/% interval, apply(data.f, 2, function(i){length(na.omit(i))})-1),
                            ...
                            df_age = match(bf5.f.no0.smooth$age2 + interval%/%2, thiele_age), dm_age = match(bf5.m.no0.smooth$age2 + interval%/%2, thiele_age),
                            df_time = match(bf5.f.no0.smooth$period2, levels(bf5.f.no0.smooth$period2)), dm_time = match(bf5.m.no0.smooth$period2, levels(bf5.m.no0.smooth$period2)),
                            ...
                            log_lambda_hypervar_prec = params$log_lambda_hyperprec,
                            log_delta_hypervar_prec = params$log_delta_hyperprec,
                            log_epsilon_hypervar_prec = params$log_epsilon_hyperprec,
...
system.time(thiele.f.loghump.oag.RW.ori.5x5 <- fit_tmb(input.thiele.loghump.oag.vec.RW,inner_verbose=TRUE, random = c("log_basepop_f","log_basepop_m",
                                                                                                                  "log_fx",
                                                                                                                  "gx_f","gx_m",
                                                                                                                  "tp_params",
                                                                                                                  "log_phi_f", "log_phi_m",
...
fit.5 <- thiele.f.loghump.oag.RW.ori.5x5$par.full %>% split(names(.))

par.vec.5 <- list(log_tau2_logpop_f = fit.5$log_tau2_logpop_f, log_tau2_logpop_m = fit.5$log_tau2_logpop_m,
                log_tau2_fx = fit.5$log_tau2_fx,
                log_tau2_gx_f = fit.5$log_tau2_gx_f,
                log_tau2_gx_m = fit.5$log_tau2_gx_m,
                log_basepop_f = log(basepop.f), log_basepop_m = log(basepop.m),
                log_fx = log_fx_mean,
                gx_f = fit.5$gx_f %>% matrix(open.age/5+1, 12) %>% 
                  apply(2, function(i){approx(x = seq(0, open.age, by = 5), y= i, xout = seq(0, open.age, by = 2), rule = 2)$y}) %>%
                  apply(1, function(i){approx(x = seq(1960, 2015, by = 5), y= i, xout = seq(1960, 2016, by = 2), rule = 2)$y}) %>%
                  c(), 
                gx_m = fit.5$gx_m %>% matrix(open.age/5+1, 12) %>% 
                  apply(2, function(i){approx(x = seq(0, open.age, by = 5), y= i, xout = seq(0, open.age, by = 2), rule = 2)$y}) %>%
                  apply(1, function(i){approx(x = seq(1960, 2015, by = 5), y= i, xout = seq(1960, 2016, by = 2), rule = 2)$y}) %>%
                  c(),
                logit_rho_g_x_f = fit.5$logit_rho_g_x_f, logit_rho_g_x_m = fit.5$logit_rho_g_x_m,
                logit_rho_g_t_f = fit.5$logit_rho_g_t_f, logit_rho_g_t_m = fit.5$logit_rho_g_t_m,
                
                log_lambda_tp = fit.5$log_lambda_tp,
                log_lambda_tp_0_inflated_sd = fit.5$log_lambda_tp_0_inflated_sd,
                tp_params = fit.5$tp_params,
                
                log_dispersion_f = fit.5$log_dispersion_f, log_dispersion_m = fit.5$log_dispersion_m,
                
                log_phi_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_phi_f, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_phi_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_phi_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_psi_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_psi_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_psi_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_psi_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_lambda_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_lambda_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_lambda_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_lambda_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_delta_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_delta_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_delta_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_delta_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_epsilon_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_epsilon_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_epsilon_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_epsilon_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_A_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_A_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_A_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_A_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                log_B_f = approx(x = seq(1960, 2015, by = 5), fit.5$log_B_f, xout = seq(1960, 2016, by = 2), rule = 2)$y, 
                log_B_m = approx(x = seq(1960, 2015, by = 5), fit.5$log_B_m, xout = seq(1960, 2016, by = 2), rule = 2)$y,
                
                log_marginal_prec_phi_f = fit.5$log_marginal_prec_phi_f, log_marginal_prec_phi_m = fit.5$log_marginal_prec_phi_m,
                log_marginal_prec_psi_f = fit.5$log_marginal_prec_psi_f, log_marginal_prec_psi_m = fit.5$log_marginal_prec_psi_m,
                log_marginal_prec_lambda_f = fit.5$log_marginal_prec_lambda_f, log_marginal_prec_lambda_m = fit.5$log_marginal_prec_lambda_m,
                log_marginal_prec_delta_f = fit.5$log_marginal_prec_delta_f, log_marginal_prec_delta_m = fit.5$log_marginal_prec_delta_m,
                log_marginal_prec_epsilon_f = fit.5$log_marginal_prec_epsilon_f, log_marginal_prec_epsilon_m = fit.5$log_marginal_prec_epsilon_m,
                log_marginal_prec_A_f = fit.5$log_marginal_prec_A_f, log_marginal_prec_A_m = fit.5$log_marginal_prec_A_m,
                log_marginal_prec_B_f = fit.5$log_marginal_prec_B_f, log_marginal_prec_B_m = fit.5$log_marginal_prec_B_m,
                
                logit_rho_phi_f = fit.5$logit_rho_phi_f, logit_rho_phi_m = fit.5$logit_rho_phi_m,
                logit_rho_psi_f = fit.5$logit_rho_psi_f, logit_rho_psi_m = fit.5$logit_rho_psi_m,
                logit_rho_A_f = fit.5$logit_rho_A_f, logit_rho_A_m = fit.5$logit_rho_A_m,
                logit_rho_B_f = fit.5$logit_rho_B_f, logit_rho_B_m = fit.5$logit_rho_B_m
)

input.thiele.loghump.oag.vec.RW <- list(data = data.loghump.vec.RW, par_init = par.vec.5, model = "ccmpp_vr_tmb")

rm(list=ls()[-which(ls()%in%c("input.thiele.loghump.oag.vec.RW", "fit_tmb", "make_tmb_obj", "fit.5"))])
gc()
...
par.func <- function(x){
  bind_rows(phi = x$mode$phi_f,
            ...
            A = x$mode$A_f,
            B = x$mode$B_f) %>%
    mutate(sex="female", period5 = bf.idx5$periods)  %>%
    bind_rows(
      bind_rows(phi = x$mode$phi_m,
                ...
                A = x$mode$A_m,
                B = x$mode$B_m) %>%
        mutate(sex="male", period5 = bf.idx5$periods)
    )
}
...
q4515.func <- function(x){
  as_tibble(apply(x$mode$mx_mat_f[15:59 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
    #as_tibble(apply(x$mode$mx_mat_f[4:12,],2,function(x){1-exp(sum(-5*x))})) %>%
    mutate(sex="female", year = bf.idx5$periods) %>%
    bind_rows(
      as_tibble(apply(x$mode$mx_mat_m[15:59 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
...
q4515.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_f[15:59 %/% interval + 1,],2,function(x){1-prod((1-0.5*x)/(1+0.5*x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "female"),
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_m[15:59 %/% interval + 1,],2,function(x){1-prod((1-0.5*x)/(1+0.5*x))})}), 1, quantile, c(0.025, 0.975))))%>% 
...
#plot 5q5 estimates####
q50.func <- function(x){
  as_tibble(apply(x$mode$mx_mat_f[0:4 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
    mutate(sex="female", year = bf.idx5$periods) %>%
    bind_rows(
      as_tibble(apply(x$mode$mx_mat_m[0:4 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})) %>%
        mutate(sex="male", year = bf.idx5$periods)
    )
}
...
q50.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_f[0:4 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "female"),
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(i$mx_mat_m[0:4 %/% interval + 1,],2,function(x){1-prod((1-0.5 * x)/(1+0.5 * x))})}), 1, quantile, c(0.025, 0.975))))%>% 
    mutate(year = bf.idx5$periods, sex = "male")
)
...
thiele.loghump <- lapply(loghump.models.list, function(y){y$mode$mx_mat_f %>%
    `colnames<-`(bf.idx5$periods) %>%  
    `rownames<-`(seq(0, nrow(y$mode$mx_mat_f)*interval - interval, by = interval)) %>%
    reshape2::melt()}) %>% 
  map2(names(.), ~ add_column(.x, model = rep(.y, nrow(.x)))) %>% 
  bind_rows() %>%
  setNames(c("age5", "period5", "value", "model")) %>%
  mutate(period5 = sprintf("%d-%d",period5, period5+interval-1),
         sex = "female") %>%
  bind_rows(
    reshape2::melt(more.countries.avg[[country]]$mort.f) %>% 
      mutate(age5 = interval * floor(Var1 / interval),
             period5 = interval * floor(Var2 / interval)) %>% 
      group_by(age5, period5) %>%
      summarise_at(vars(value), mean) %>%
      mutate(value = exp(value),
             period5 = sprintf("%d-%d",period5, period5+interval-1),
...
DHS.plot <- bf5.f.no0.smooth %>% mutate(value = adjusted / pyears2, period2 = as.numeric(levels(period2)[period2])) %>%
  select(tips, age2, period2, value) %>%
  mutate(period5 = sprintf("%d-%d", period2, period2 + interval-1),
         sex = "female") %>%
  bind_rows(
    bf5.m.no0.smooth %>% mutate(value = adjusted / pyears2, period2 = as.numeric(levels(period2)[period2])) %>%
      select(tips, age2, period2, value) %>%
      mutate(period5 = sprintf("%d-%d", period2, period2 + interval-1),
             sex = "male") 
  )
...
mx.var.df.f <- as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$mx_mat_f}), 1, quantile, c(0.025, 0.975)))) %>%
  mutate(sex = "female",
         age5 = rep(thiele_age - interval%/%2, bf.idx5$n_periods),
         period = rep(bf.idx5$periods, each = length(thiele_age)),
         period5 = sprintf("%d-%d", period, period + interval-1))
...
ggplot(thiele.f %>% filter(age5 %in% 0:70, hump=="log-Normal hump")) + geom_line(aes(x = age5, y = value, col=model), lwd=1.2, linetype = 2) +
  ...
  geom_point(data = filter(DHS.plot.f, value!=0), aes(x = age2, y = value, shape = tips), alpha = 0) + 
  geom_point(data = filter(DHS.plot.f, value!=0, tips %in% 1:3), aes(x = age2, y = value, shape = tips), size=2.5, stroke=1.5) +
  geom_point(data = filter(DHS.plot.f, value!=0, !tips %in% 1:3), aes(x = age2, y = value, shape = tips), size=1.5, alpha = 0.3) +
  ...
  ggtitle(paste(country, "Females (log-Normal hump)")) + ylab(bquote(""[.(interval)]*m[x])) + xlab(paste0(interval,"-year age groups")) +
  theme(text = element_text(size=25),
        plot.title = element_text(hjust = 0.5, face = "bold", size = 35)) +
  facet_wrap(~period5)
...
ggplot(thiele.m %>% filter(age5 %in% 0:70, hump == "log-Normal hump")) + geom_line(aes(x = age5, y = value, col=model), lwd=1.2, linetype = 2) +
  ...
  geom_point(data = filter(DHS.plot.m, value!=0), aes(x = age2, y = value, shape = tips), alpha = 0) + 
  geom_point(data = filter(DHS.plot.m, value!=0, tips %in% 1:3), aes(x = age2, y = value, shape = tips), size=2.5, stroke=1.5) +
  geom_point(data = filter(DHS.plot.m, value!=0, !tips %in% 1:3), aes(x = age2, y = value, shape = tips), size=1.5, alpha = 0.3) +
  geom_ribbon(data = mx.var.df.m, aes(x = age5, ymin = `2.5%`, ymax = `97.5%`), alpha=0.2, fill = 4) +
  scale_shape_manual(values = 0:14) +
  scale_y_continuous(trans = "log", labels = function(x){as.character(round(x,3))}) +
  facet_wrap(~period5) + 
  ggtitle(paste(country, "Males (log-Normal hump)")) + ylab(bquote(""[.(interval)]*m[x])) + xlab(paste0(interval,"-year age groups")) +
  theme(text = element_text(size=25),
        plot.title = element_text(hjust = 0.5, face = "bold", size = 35))
...
ggplot(bind_rows(thiele.f, thiele.m) %>% filter(age5 %in% 0:70, hump=="log-Normal hump")) + geom_line(aes(x = period, y = value, col=sex, linetype=model), lwd=1.2) +
  ...
  ggtitle(paste(country, "Estimate Mortality Schedules")) + ylab(bquote(""[.(interval)]*m[x])) + xlab(paste0(interval,"-year age groups")) +
  scale_color_manual(values = c("red", "blue")) +
  scale_fill_manual(values = c("red", "blue")) +
  theme(text = element_text(size=25),
        plot.title = element_text(hjust = 0.5, face = "bold", size = 35)) +
...
get.pop<- function(x){
  ...
  pop.mat.m <- matrix(x$mode$population_m, bf.idx5$n_ages, bf.idx5$n_periods+1)
  rownames(pop.mat) <-  rownames(pop.mat.m) <- c(bf.idx5$ages)
  colnames(pop.mat) <- colnames(pop.mat.m) <- bf.idx5$periods_out
  bind_rows(
...
pop.df <- lapply(loghump.models.list, get.pop) %>% 
  ...
  bind_rows(
    ...
    bind_rows(ddharm_bf_census_f_oag %>% mutate(sex = "female"),
              ddharm_bf_census_m_oag %>% mutate(sex = "male")) %>% 
  ...
  mutate(cohort = year - age,
         cohort = interval * floor(cohort / interval),
...
pop.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$population_f}), 1, quantile, c(0.025, 0.975))))%>%
    mutate(sex = "female",
           age = rep(bf.idx5$ages, bf.idx5$n_periods + 1),
           year = rep(bf.idx5$periods_out, each = bf.idx5$n_ages),
...
get.census.pop<- function(x){
  ...
  pop.mat.m <- matrix(x$mode$census_proj_mat_m, bf.idx5$n_ages, ncol(data.f))
  rownames(pop.mat) <-  rownames(pop.mat.m) <- c(bf.idx5$ages)
...
censpop.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$census_proj_mat_f}), 1, quantile, c(0.025, 0.975))))%>%
    mutate(sex = "female",
           age = rep(bf.idx5$ages, ncol(data.f)),
           year = rep(as.numeric(colnames(data.f)), each = bf.idx5$n_ages),
  ...
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$census_proj_mat_m}), 1, quantile, c(0.025, 0.975))))%>%
    mutate(sex = "male",
           age = rep(bf.idx5$ages, ncol(data.m)),
           year = rep(as.numeric(colnames(data.m)), each = bf.idx5$n_ages),
           period5 = sprintf("%d-%d", year, year + 4))
)
...
get.mig<- function(x){
  ...
  pop.mat.m <- matrix(x$mode$migrations_m, bf.idx5$n_ages, bf.idx5$n_periods)
  rownames(pop.mat) <- rownames(pop.mat.m) <- c(bf.idx5$ages)
  colnames(pop.mat) <- colnames(pop.mat.m) <- bf.idx5$periods
...
mig.var.df <- bind_rows(
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$migrations_f/i$population_f[1:(bf.idx5$n_ages*bf.idx5$n_periods)]}), 1, quantile, c(0.025, 0.975)))) %>%
    mutate(sex = "female",
           age = rep(bf.idx5$ages, bf.idx5$n_periods),
           year = rep(bf.idx5$periods, each = bf.idx5$n_ages),
           period5 = sprintf("%d-%d", year, year + 4)),
  
  as_tibble(t(apply(sapply(thiele.var.sim, function(i){i$migrations_m/i$population_m[1:(bf.idx5$n_ages*bf.idx5$n_periods)]}), 1, quantile, c(0.025, 0.975)))) %>%
    mutate(sex = "male",
           age = rep(bf.idx5$ages, bf.idx5$n_periods),
           year = rep(bf.idx5$periods, each = bf.idx5$n_ages),
           period5 = sprintf("%d-%d", year, year + 4))
...
inner_join(mig.df, pop.df) %>% mutate(gx = mig/value, model = fct_relevel(model, "Thiele RW")) %>%
  filter(year %in% c(interval * seq(min(bf.idx5$periods), max(bf.idx5$periods), length = 6) %/% interval)) %>%
  ggplot() + geom_line(aes(x = age, y = gx, col = sex, linetype=model), lwd = 1.2) +
  geom_ribbon(data = mig.var.df %>% filter(year %in% c(interval * seq(min(bf.idx5$periods), max(bf.idx5$periods), length = 6) %/% interval)),
              aes(x = age, ymin = `2.5%`, ymax = `97.5%`, fill = sex), alpha=0.2) +
...
inner_join(mig.df, pop.df) %>% mutate(gx = mig/value, model = fct_relevel(model, "Thiele RW")) %>%
  ...
  theme(text = element_text(size=25), legend.key.size = unit(1.5,"cm"), strip.text = element_text(size=30),
        plot.title = element_text(hjust = 0.5, face = "bold", size = 35)) +
  facet_wrap_paginate(~cohort, nrow=2, ncol=2, page=10)
...
tfr.df <- fx.df %>%
  ...
  summarise_at(vars(value), function(i){interval * sum(i)})

tfr.var.df <- as_tibble(t(apply(sapply(thiele.var.sim, function(i){apply(matrix(i$fx, bf.idx5$n_fx, bf.idx5$n_periods), 2, function(j){interval * sum(j)})}), 1, quantile, c(0.025, 0.975)))) %>%
