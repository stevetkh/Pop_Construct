...
Type objective_function<Type>::operator() ()
{
  ...
  //DATA_SPARSE_MATRIX(penal_time_1);
  DATA_SPARSE_MATRIX(null_penal_time);
  DATA_VECTOR(spline_params_timescale);
  DATA_MATRIX(spline_params_errmat);
  ...
  PARAMETER(log_lambda_f_intercept);
  PARAMETER(log_delta_f_intercept);
  PARAMETER(log_epsilon_f_intercept);

  PARAMETER(log_lambda_m_intercept);
  PARAMETER(log_delta_m_intercept);
  PARAMETER(log_epsilon_m_intercept);

  PARAMETER(log_lambda_f_slope);
  PARAMETER(log_delta_f_slope);
  PARAMETER(log_epsilon_f_slope);

  PARAMETER(log_lambda_m_slope);
  PARAMETER(log_delta_m_slope);
  PARAMETER(log_epsilon_m_slope);

  PARAMETER_VECTOR(log_lambda_f_err);
  PARAMETER_VECTOR(log_delta_f_err);
  PARAMETER_VECTOR(log_epsilon_f_err);

  PARAMETER_VECTOR(log_lambda_m_err);
  PARAMETER_VECTOR(log_delta_m_err);
  PARAMETER_VECTOR(log_epsilon_m_err);
  ...
  PARAMETER(log_lambda_phi_f);
  PARAMETER(log_lambda_psi_f);
  PARAMETER(log_lambda_lambda_f);
  PARAMETER(log_lambda_delta_f);
  PARAMETER(log_lambda_epsilon_f);
  PARAMETER(log_lambda_A_f);
  PARAMETER(log_lambda_B_f);

  PARAMETER(log_lambda_phi_m);
  PARAMETER(log_lambda_psi_m);
  PARAMETER(log_lambda_lambda_m);
  PARAMETER(log_lambda_delta_m);
  PARAMETER(log_lambda_epsilon_m);
  ...
  nll -= dlgamma(log_lambda_phi_f, Type(1.0), Type(exp(log_phi_hyperlambda) / 2.0), true);
  nll -= dlgamma(log_lambda_phi_m, Type(1.0), Type(exp(log_phi_hyperlambda) / 2.0), true);

  nll -= dlgamma(log_lambda_psi_f, Type(1.0), Type(exp(log_psi_hyperlambda) / 2.0), true);
  nll -= dlgamma(log_lambda_psi_m, Type(1.0), Type(exp(log_psi_hyperlambda) / 2.0), true);

  nll -= dlgamma(log_lambda_lambda_f, Type(1.0), Type(exp(log_lambda_hyperlambda) / 2.0), true);
  Type sigma_lambda_f = exp(-0.5 * log_lambda_lambda_f);
  nll -= dlgamma(log_lambda_lambda_m, Type(1.0), Type(exp(log_lambda_hyperlambda) / 2.0), true);
  Type sigma_lambda_m = exp(-0.5 * log_lambda_lambda_m);

  nll -= dlgamma(log_lambda_delta_f, Type(1.0), Type(exp(log_delta_hyperlambda) / 2.0), true);
  Type sigma_delta_f = exp(-0.5 * log_lambda_delta_f);
  nll -= dlgamma(log_lambda_delta_m, Type(1.0), Type(exp(log_delta_hyperlambda) / 2.0), true);
  Type sigma_delta_m = exp(-0.5 * log_lambda_delta_m);

  nll -= dlgamma(log_lambda_epsilon_f, Type(1.0), Type(exp(log_epsilon_hyperlambda) / 2.0), true);
  Type sigma_epsilon_f = exp(-0.5 * log_lambda_epsilon_f);
  nll -= dlgamma(log_lambda_epsilon_m, Type(1.0), Type(exp(log_epsilon_hyperlambda) / 2.0), true);
  Type sigma_epsilon_m = exp(-0.5 * log_lambda_epsilon_m);
  ...
  nll -= dlgamma(log_lambda_B_f, Type(1.0), Type(exp(log_B_hyperlambda) / 2.0), true);
  nll -= dlgamma(log_lambda_B_m, Type(1.0), Type(exp(log_B_hyperlambda) / 2.0), true);
  ...
  SparseMatrix<Type> QQ_phi_f =  exp(log_lambda_phi_f) * penal_time + exp(log_marginal_prec_phi_f) * null_penal_time;
  ...
  SparseMatrix<Type> QQ_B_f =  exp(log_lambda_B_f) * penal_time + exp(log_marginal_prec_B_f) * null_penal_time;
  nll += GMRF(QQ_B_f)(log_B_f_spline_params);

  nll -= dnorm(log_lambda_f_intercept, log_lambda_mean_f, Type(2.0), true);
  nll -= dnorm(log_lambda_f_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_lambda_f_err, Type(0.0), sigma_lambda_f, true).sum();

  nll -= dnorm(log_delta_f_intercept, log_delta_mean_f, Type(0.3), true);
  nll -= dnorm(log_delta_f_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_delta_f_err, Type(0.0), sigma_delta_f, true).sum();

  nll -= dnorm(log_epsilon_f_intercept, log_epsilon_mean_f, Type(0.5), true);
  nll -= dnorm(log_epsilon_f_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_epsilon_f_err, Type(0.0), sigma_epsilon_f, true).sum();

  SparseMatrix<Type> QQ_phi_m =  exp(log_lambda_phi_m) * penal_time + exp(log_marginal_prec_phi_m) * null_penal_time;
  ...
  SparseMatrix<Type> QQ_B_m =  exp(log_lambda_B_m) * penal_time + exp(log_marginal_prec_B_m) * null_penal_time;
  nll += GMRF(QQ_B_m)(log_B_m_spline_params);

  nll -= dnorm(log_lambda_m_intercept, log_lambda_mean_m, Type(2.0), true);
  nll -= dnorm(log_lambda_m_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_lambda_m_err, Type(0.0), sigma_lambda_m, true).sum();

  nll -= dnorm(log_delta_m_intercept, log_delta_mean_m, Type(0.3), true);
  nll -= dnorm(log_delta_m_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_delta_m_err, Type(0.0), sigma_delta_m, true).sum();

  nll -= dnorm(log_epsilon_m_intercept, log_epsilon_mean_m, Type(0.5), true);
  nll -= dnorm(log_epsilon_m_slope, Type(0.0), Type(0.01), true);
  nll -= dnorm(log_epsilon_m_err, Type(0.0), sigma_epsilon_m, true).sum();

  vector<Type> log_lambda_f_spline_params = log_lambda_f_intercept + log_lambda_f_slope * spline_params_timescale + spline_params_errmat * log_lambda_f_err;
  vector<Type> log_delta_f_spline_params = log_delta_f_intercept + log_delta_f_slope * spline_params_timescale + spline_params_errmat * log_delta_f_err;
  vector<Type> log_epsilon_f_spline_params = log_epsilon_f_intercept + log_epsilon_f_slope * spline_params_timescale + spline_params_errmat * log_epsilon_f_err;

  vector<Type> log_lambda_m_spline_params = log_lambda_m_intercept + log_lambda_m_slope * spline_params_timescale + spline_params_errmat * log_lambda_m_err;
  vector<Type> log_delta_m_spline_params = log_delta_m_intercept + log_delta_m_slope * spline_params_timescale + spline_params_errmat * log_delta_m_err;
  vector<Type> log_epsilon_m_spline_params = log_epsilon_m_intercept + log_epsilon_m_slope * spline_params_timescale + spline_params_errmat * log_epsilon_m_err;
  ...
  SparseMatrix<Type> QQ_gx_f = exp(log_lambda_gx_age_f) * penal_age_gx + exp(log_lambda_gx_time_f) * penal_time_gx + exp(log_lambda_gx_agetime_f) * penal_agetime_gx + exp(log_tau2_gx_f) * null_penal_gx;
  SparseMatrix<Type> QQ_gx_m = exp(log_lambda_gx_age_m) * penal_age_gx + exp(log_lambda_gx_time_m) * penal_time_gx + exp(log_lambda_gx_agetime_f) * penal_agetime_gx + exp(log_tau2_gx_m) * null_penal_gx;
