...
par.vec <- list(log_tau2_logpop_f = c(2,4), log_tau2_logpop_m = c(2,4),
                ...
                log_lambda_f = rep(log(0.003), bf.idx5$n_periods), log_lambda_m = rep(log(0.003), bf.idx5$n_periods),
  ...
  bind_rows(
    thiele.loghump.prior.f %>%
      ...
      mutate(sex = "female", model = "Initial Values",
             value = replace(value, Var1=="lambda", 0.005),
             value = replace(value, Var1=="delta", 0.7),
             value = replace(value, Var1=="epsilon", 20)) %>%
    ...
    thiele.loghump.prior.m %>%
      ...
      mutate(sex = "male", model = "Initial Values",
             value = replace(value, Var1=="lambda", 0.005),
             value = replace(value, Var1=="delta", 0.7),
             value = replace(value, Var1=="epsilon", 25)) %>%
      select(name = Var1, period5 = Var2, value, model, sex)
  ) %>%
...
q4515.df.loghump <-  lapply(loghump.models.list, q4515.func) %>% 
  ...
  bind_rows(
    ...
    apply(apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * (seq(17, 57, by = 5)-2)) + 
        0.005 * exp(-0.7 * ((log(seq(17, 57, by = 5)) - log(20))^2)) +
    ...
    apply(apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * (seq(17, 57, by = 5)-2)) + 
        0.005 * exp(-0.7 * ((log(seq(17, 57, by = 5)) - log(25))^2)) +
        i[6] * exp(i[7] * (seq(17, 57, by = 5)-92))}) %>%
          `colnames<-`(bf.idx5$periods) %>% 
          `rownames<-`(seq(15, 55, by = 5)), 2, function(j){1-prod((1-2.5*j)/(1+2.5*j))}) %>%
      as_tibble() %>%
...
q50.df.loghump <- lapply(loghump.models.list, q50.func) %>% 
  ...
  bind_rows(
    ...
    as_tibble(5 * apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * 0) + 0.005 * exp(-0.7 * (log(2) - log(20))^2) + i[6] * exp(i[7] * (2-92))}) / 
                (1 + 2.5 * apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * 0) + 0.005 * exp(-0.7 * (log(2) - log(25))^2) + i[6] * exp(i[7] * (2-92))}) )) %>%
      mutate(model="Initial Values", year=bf.idx5$periods, variable = NULL, sex = "female"),
    as_tibble(5 * apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * 0) + 0.005 * exp(-0.7 * (log(2) - log(25))^2) + i[6] * exp(i[7] * (2-92))}) / 
                (1 + 2.5 * apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * 0) + 0.005 * exp(-0.7 * (log(2) - log(25))^2) + i[6] * exp(i[7] * (2-92))}) )) %>%
  ...
  bind_rows(
    lapply(
      list(Child = apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5))}),
           Hump = apply(thiele.loghump.prior.f, 2, function(i) {0.005 * exp(-0.7 * ((log(seq(2, 112, by = 5)) - log(20))^2))}),
           ...
           Total = apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5)) + 
               0.005 * exp(-0.7 * ((log(seq(2, 112, by = 5)) - log(20))^2)) +
    ...
    lapply(
      list(Child = apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5))}),
           Hump = apply(thiele.loghump.prior.m, 2, function(i) {0.005 * exp(-0.7 * ((log(seq(2, 112, by = 5)) - log(25))^2))}),
           ...
           Total = apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5)) + 
               0.005 * exp(-0.7 * ((log(seq(2, 112, by = 5)) - log(25))^2)) +
               i[6] * exp(i[7] * seq(-90, 20, by = 5))})
      ), function(i) {
        i %>%
          `colnames<-`(bf.idx5$periods) %>%  
