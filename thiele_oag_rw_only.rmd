...
params:
  ...
  log_lambda_hyperprec: log(2/0.01)
  log_delta_hyperprec: log(2/0.01)
  log_epsilon_hyperprec: log(2/0.01)
...
data.loghump.vec.RW <- list(log_basepop_mean_f = log(basepop.f), log_basepop_mean_m = log(basepop.m),
                            ...
                            log_lambda_mean_f = log(params$lambda_f), log_lambda_mean_m = log(params$lambda_m),
                            log_delta_mean_f = log(params$delta_f), log_delta_mean_m = log(params$delta_m),
                            log_epsilon_mean_f = log(params$epsilon_f), log_epsilon_mean_m = log(params$epsilon_m),
...
par.vec <- list(log_tau2_logpop_f = c(2,4), log_tau2_logpop_m = c(2,4),
                ...
                log_lambda_f = rep(log(params$lambda_f), bf.idx5$n_periods), log_lambda_m = rep(log(params$lambda_m), bf.idx5$n_periods),
                log_delta_f = rep(log(params$delta_f), bf.idx5$n_periods), log_delta_m = rep(log(params$delta_m), bf.idx5$n_periods),
                log_epsilon_f = rep(log(params$epsilon_f), bf.idx5$n_periods), log_epsilon_m = rep(log(params$epsilon_m), bf.idx5$n_periods),
                log_A_f = log(thiele.loghump.prior.f[6,]), log_A_m = log(thiele.loghump.prior.f[6,]),
                log_B_f = log(thiele.loghump.prior.f[7,]), log_B_m = log(thiele.loghump.prior.f[7,]),
...
```{r, include=FALSE}
loghump.models.list <- list("Thiele RW" = thiele.f.loghump.oag.RW.ori, "Thiele Restricted Data RW" = thiele.f.loghump.oag.RW.ori.re)
```
  ...
  bind_rows(
    thiele.loghump.prior.f %>%
      ...
      mutate(sex = "female", model = "Initial Values",
             value = replace(value, Var1=="lambda", params$lambda_f),
             value = replace(value, Var1=="delta", params$delta_f),
             value = replace(value, Var1=="epsilon", params$epsilon_f)) %>%
      select(name = Var1, period5 = Var2, value, model, sex),
    thiele.loghump.prior.m %>%
      `colnames<-`(bf.idx5$periods) %>%
      reshape2::melt() %>%
      mutate(sex = "male", model = "Initial Values",
             value = replace(value, Var1=="lambda", params$lambda_m),
             value = replace(value, Var1=="delta", params$delta_m),
             value = replace(value, Var1=="epsilon", params$epsilon_m)) %>%
      select(name = Var1, period5 = Var2, value, model, sex)
  ) %>%
...
\newpage
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap = "Estimated parameters"}
...
q4515.df.loghump <-  lapply(loghump.models.list, q4515.func) %>% 
  ...
  bind_rows(
    ...
    apply(apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * (seq(17, 57, by = 5)-2)) + 
        params$lambda_f * exp(- params$delta_f * ((log(seq(17, 57, by = 5)) - log(params$epsilon_f))^2)) +
    ...
    apply(apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * (seq(17, 57, by = 5)-2)) + 
        params$lambda_m * exp(- params$delta_m * ((log(seq(17, 57, by = 5)) - log(params$epsilon_m))^2)) +
        i[6] * exp(i[7] * (seq(17, 57, by = 5)-92))}) %>%
          `colnames<-`(bf.idx5$periods) %>% 
          `rownames<-`(seq(15, 55, by = 5)), 2, function(j){1-prod((1-2.5*j)/(1+2.5*j))}) %>%
      as_tibble() %>%
...
q50.df.loghump <- lapply(loghump.models.list, q50.func) %>% 
  ...
  bind_rows(
    ...
    as_tibble(5 * apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * 0) + params$lambda_f * exp(- params$delta_f * (log(2) - log(params$epsilon_f))^2) + i[6] * exp(i[7] * (2-92))}) / 
                (1 + 2.5 * apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * 0) + params$lambda_f * exp(- params$delta_f * (log(2) - log(params$epsilon_f))^2) + i[6] * exp(i[7] * (2-92))}) )) %>%
      mutate(model="Initial Values", year=bf.idx5$periods, variable = NULL, sex = "female"),
    as_tibble(5 * apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * 0) + params$lambda_m * exp(- params$delta_m * (log(2) - log(params$epsilon_m))^2) + i[6] * exp(i[7] * (2-92))}) / 
                (1 + 2.5 * apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * 0) + params$lambda_m * exp(- params$delta_m * (log(2) - log(params$epsilon_m))^2) + i[6] * exp(i[7] * (2-92))}) )) %>%
  ...
  bind_rows(
    lapply(
      list(Child = apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5))}),
           Hump = apply(thiele.loghump.prior.f, 2, function(i) {params$lambda_f * exp(- params$delta_f * ((log(seq(2, 112, by = 5)) - log(params$epsilon_f))^2))}),
           ...
           Total = apply(thiele.loghump.prior.f, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5)) + 
               params$lambda_f * exp(- params$delta_f * ((log(seq(2, 112, by = 5)) - log(params$epsilon_f))^2)) +
    ...
    lapply(
      list(Child = apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5))}),
           Hump = apply(thiele.loghump.prior.m, 2, function(i) {params$lambda_m * exp(- params$delta_m * ((log(seq(2, 112, by = 5)) - log(params$epsilon_m))^2))}),
           ...
           Total = apply(thiele.loghump.prior.m, 2, function(i) {i[1] * exp(-i[2] * seq(0, 110, by = 5)) + 
               params$lambda_m * exp(- params$delta_m * ((log(seq(2, 112, by = 5)) - log(params$epsilon_m))^2)) +
               i[6] * exp(i[7] * seq(-90, 20, by = 5))})
      ), function(i) {
        i %>%
          `colnames<-`(bf.idx5$periods) %>%  
