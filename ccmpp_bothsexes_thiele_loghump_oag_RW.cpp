...
Type objective_function<Type>::operator() ()
{
  ...
  log_lambda_f(0) = Type(2.5) * log_lambda_innov_f(0);
  log_delta_f(0) = Type(0.3) * log_delta_innov_f(0);
  log_epsilon_f(0) = Type(1.0) * log_epsilon_innov_f(0);
  ...
  log_lambda_m(0) = Type(2.5) * log_lambda_innov_m(0);
  log_delta_m(0) = Type(0.3) * log_delta_innov_m(0);
  log_epsilon_m(0) = Type(1.0) * log_epsilon_innov_m(0);
  ...
  for(int i=1; i < log_phi_innov_f.size(); i++){
    ...
    log_lambda_f(i) = log_lambda_f(i-1) + sigma_lambda_f * log_lambda_innov_f(i);
    log_delta_f(i) = log_delta_f(i-1) + sigma_delta_f * log_delta_innov_f(i);
    log_epsilon_f(i) = log_epsilon_f(i-1) + sigma_epsilon_f * log_epsilon_innov_f(i);
    ...
    log_lambda_m(i) = log_lambda_m(i-1) + sigma_lambda_m * log_lambda_innov_m(i);
    log_delta_m(i) = log_delta_m(i-1) + sigma_delta_m * log_delta_innov_m(i);
    log_epsilon_m(i) = log_epsilon_m(i-1) + sigma_epsilon_m * log_epsilon_innov_m(i);
