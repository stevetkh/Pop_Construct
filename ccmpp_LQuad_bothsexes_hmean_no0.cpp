...
Type objective_function<Type>::operator() ()
{
  ...
  h_params_iid_f(i) = h_params_f(i) - rho_h(0) * h_params_f(i-1);
  h_params_iid_m(i) = h_params_m(i) - rho_h(1) * h_params_m(i-1);
  }

  vector<Type> h_iid_mean_f = (1.0 - rho_h(0)) * h_constant_f;
  vector<Type> h_iid_mean_m = (1.0 - rho_h(1)) * h_constant_m;
  h_iid_mean_f(0) += rho_h(0) * h_constant_f(0);
  h_iid_mean_m(0) += rho_h(1) * h_constant_m(0);

  h_params_iid_f -= h_iid_mean_f;
  h_params_iid_m -= h_iid_mean_m;

  h_params_iid_f *= 1/sqrt(1 - rho_h(0) * rho_h(0));
  h_params_iid_f(0) *= sqrt(1 - rho_h(0) * rho_h(0));
  h_params_iid_m *= 1/sqrt(1 - rho_h(1) * rho_h(1));
  h_params_iid_m(0) *= sqrt(1 - rho_h(1) * rho_h(1));

  nll -= dnorm(h_params_iid_f, Type(0.0), sigma_h(0), 1).sum();
  nll -= dnorm(h_params_iid_m, Type(0.0), sigma_h(1), 1).sum();
  nll -= (0.5 - h_params_f.size() / 2) * log(1.0 - rho_h(0) * rho_h(0));
  nll -= (0.5 - h_params_m.size() / 2) * log(1.0 - rho_h(1) * rho_h(1));
